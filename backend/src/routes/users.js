...
// Profildaten des angemeldeten Benutzers abrufen
...
router.get('/current', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: 'Fehler beim Abrufen des aktuellen Benutzers',
      error: error.message,
    });
  }
});
...
router.get('/current/teams', authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Fehler beim Abrufen der Teams', error: error.message });
  }
});
...
router.put('/profile/update', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      logger.warn(
        `Failed to update own profile for user ${userId}: ${result.message}`
      );
      res.status(400).json({ message: result.message });
    }
  } catch (error) {
    logger.error(
      `Error updating own profile for user ${req.user.id}: ${error.message}`
    );
    res.status(500).json({
      message: 'Fehler beim Aktualisieren des eigenen Profils',
      error: error.message,
    });
  }
});
...
router.put('/change-password', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      logger.warn(
        `Failed to change password for user ${userId}: ${result.message}`
      );
      res.status(400).json({ message: result.message });
    }
  } catch (error) {
    logger.error(
      `Error changing password for user ${req.user.id}: ${error.message}`
    );
    res.status(500).json({
    ...
    try {
      ...
      if (user && user.profile_picture) {
        try {
          ...
          logger.warn(
            `Could not delete old profile picture: ${unlinkError.message}`
          );
        }
      }
      ...
      if (success) {
        ...
        res
          .status(500)
          .json({ message: 'Fehler beim Aktualisieren des Profilbilds' });
      }
    } catch (error) {
...
router.delete('/profile/picture', authenticateToken, async (req, res) => {
  try {
    ...
    if (success) {
      ...
      res
        .status(500)
        .json({ message: 'Fehler beim Entfernen des Profilbilds' });
    }
  } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (success) {
        ...
        res
          .status(500)
          .json({ message: 'Fehler beim Aktualisieren des Benutzers' });
      }
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (user.is_archived) {
        return res
          .status(400)
          .json({ message: 'Benutzer ist bereits archiviert' });
      }
      ...
      if (success) {
        ...
        res
          .status(500)
          .json({ message: 'Fehler beim Archivieren des Benutzers' });
      }
    } catch (error) {
      logger.error(`Error archiving user ${req.params.id}: ${error.message}`);
      res.status(500).json({
        message: 'Fehler beim Archivieren des Benutzers',
        error: error.message,
      });
    }
  }
  ...
  async (req, res) => {
    try {
      ...
      if (!user.is_archived) {
        return res
          .status(400)
          .json({ message: 'Benutzer ist nicht archiviert' });
      }
      ...
      if (success) {
        ...
        res
          .status(500)
          .json({ message: 'Fehler beim Aufheben der Benutzer-Archivierung' });
      }
    } catch (error) {
      logger.error(`Error unarchiving user ${req.params.id}: ${error.message}`);
      res.status(500).json({
        message: 'Fehler beim Aufheben der Benutzer-Archivierung',
        error: error.message,
      });
    }
  }
...
router.get('/:id/teams', authenticateToken, checkOwnUser, async (req, res) => {
  try {
    ...
    logger.error(
      `Error fetching teams for user ${req.params.id}: ${error.message}`
    );
    res
      .status(500)
      .json({ message: 'Fehler beim Abrufen der Teams', error: error.message });
