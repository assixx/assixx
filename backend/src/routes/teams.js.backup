...
// GET routes are allowed for all authenticated users (read-only)
// Other methods require admin/root role
...
router.get('/:id', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Fehler beim Abrufen des Teams', error: error.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Fehler beim Löschen des Teams', error: error.message });
  }
});
...
router.post('/:id/members', async (req, res) => {
  try {
    ...
    if (success) {
      logger.info(
        `User ${userId} added to team ${teamId} by user ${req.user.username}`
      );
...
router.delete('/:id/members/:userId', async (req, res) => {
  try {
    ...
    if (success) {
      logger.info(
        `User ${userId} removed from team ${teamId} by user ${req.user.username}`
      );
