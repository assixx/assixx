...
router.post('/', async (req, res) => {
  try {
    ...
    if (!name) {
      return res
        .status(400)
        .json({ message: 'Abteilungsname ist erforderlich' });
    }
    ...
    logger.info(
      `Department created with ID ${departmentId} by user ${req.user.username}`
    );
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (success) {
      logger.info(
        `Department ${departmentId} updated by user ${req.user.username}`
      );
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (success) {
      logger.info(
        `Department ${departmentId} deleted by user ${req.user.username}`
      );
