...
router.get(
  ...
  // codeql[js/missing-rate-limiting] - False positive: Rate limiting is applied via rateLimiter.authenticated middleware
  async (req, res) => kvpController.getAll(req, res),
);
...
router.get('/stats', rateLimiter.admin, checkRole(['admin', 'root']), async (req, res) =>
  kvpController.getStatistics(req, res),
);
router.get('/:id', rateLimiter.authenticated, async (req, res) =>
  kvpController.getById(req as unknown as Parameters<typeof kvpController.getById>[0], res),
);

// Creation (employees and admins in employee mode)
router.post('/', rateLimiter.authenticated, async (req, res) =>
  kvpController.create(req as unknown as Parameters<typeof kvpController.create>[0], res),
);

// Updates (based on permissions)
router.put('/:id', rateLimiter.authenticated, async (req, res) =>
  kvpController.update(req as unknown as Parameters<typeof kvpController.update>[0], res),
);

// Archive (soft delete)
router.delete('/:id', rateLimiter.authenticated, async (req, res) =>
  kvpController.delete(req as unknown as Parameters<typeof kvpController.delete>[0], res),
);
...
router.get('/:id/comments', rateLimiter.authenticated, async (req, res) =>
  kvpController.getComments(req as unknown as Parameters<typeof kvpController.getComments>[0], res),
);
router.post('/:id/comments', rateLimiter.authenticated, async (req, res) =>
  kvpController.addComment(req as unknown as Parameters<typeof kvpController.addComment>[0], res),
);
