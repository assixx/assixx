...
describe('Blackboard API Routes', () => {
  describe('GET /api/blackboard', () => {
    it('should fetch all blackboard entries', async () => {
      ...
      (blackboardModel.getAllEntries as jest.Mock).mockResolvedValue(mockEntries);
    ...
    it('should handle query parameters correctly', async () => {
      ...
      await request(app)
        .get('/api/blackboard?status=archived&filter=department&search=test&page=2&limit=20')
        .expect(200);
  ...
  describe('POST /api/blackboard', () => {
    it('should create a company-level entry', async () => {
      ...
      (blackboardModel.createEntry as jest.Mock).mockResolvedValue(createdEntry);

      const response = await request(app).post('/api/blackboard').send(newEntry).expect(201);
    ...
    it('should handle missing required fields', async () => {
      ...
      const response = await request(app).post('/api/blackboard').send(invalidEntry).expect(500);
  ...
  describe('PUT /api/blackboard/:id', () => {
    it('should update an entry', async () => {
      ...
      (blackboardModel.updateEntry as jest.Mock).mockResolvedValue(updatedEntry);

      const response = await request(app).put('/api/blackboard/1').send(updateData).expect(200);
  ...
  describe('DELETE /api/blackboard/:id', () => {
    it('should delete an entry', async () => {
      ...
      const response = await request(app).delete('/api/blackboard/1').expect(200);
  ...
  describe('POST /api/blackboard/:id/confirm', () => {
    it('should confirm an entry', async () => {
      ...
      const response = await request(app).post('/api/blackboard/1/confirm').expect(200);
  ...
  describe('Permission Tests', () => {
    ...
    it('should restrict non-admin users appropriately', async () => {
      ...
      expect(response.body.message).toContain('Only admins can create company-wide entries');
    });
  });
});
