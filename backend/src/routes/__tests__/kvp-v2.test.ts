...
describe("KVP API v2", () => {
  ...
  beforeEach(async () => {
    ...
    const [categoryResult] = await testDb.execute<ResultSetHeader>(
      "INSERT INTO kvp_categories (tenant_id, name, description, color, icon) VALUES (?, ?, ?, ?, ?)",
      [tenantId, "Productivity", "Productivity improvements", "#3498db", "💡"],
    );
  ...
  describe("Dashboard Statistics", () => {
    beforeEach(async () => {
      ...
      await testDb.execute(
        ...
        (?, 'In Progress', 'Desc', ?, 'company', 0, ?, 'high', 'in_progress', NULL, NOW(), NOW()),
