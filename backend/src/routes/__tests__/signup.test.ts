...
describe('Signup API Endpoints', () => {
  ...
  describe('POST /api/signup', () => {
    it('should successfully create a new tenant with admin user', async () => {
      const response = await request(app).post('/api/signup').send(validSignupData);
      ...
      const [rows] = await testDb.execute('SELECT * FROM tenants WHERE subdomain = ?', [
      ...
      const [allUserRows] = await testDb.execute('SELECT * FROM users WHERE tenant_id = ?', [
        tenantId,
      ]);
    ...
    it('should create tenant-specific database entries', async () => {
      ...
      const [rows] = await testDb.execute('SELECT id FROM tenants WHERE subdomain = ?', ['dbtest']);
      const tenants = asTestRows<unknown>(rows);
      const tenantId = tenants[0].id;

      // Check default department was created
      const [deptRows] = await testDb.execute('SELECT * FROM departments WHERE tenant_id = ?', [
        tenantId,
      ]);
  ...
  describe('GET /api/check-subdomain/:subdomain', () => {
    ...
    it('should return available for unused subdomain', async () => {
      const response = await request(app).get('/api/check-subdomain/newcompany');
    ...
    it('should validate subdomain format', async () => {
      const response = await request(app).get('/api/check-subdomain/Invalid-Subdomain');
    ...
    it('should reject reserved subdomains', async () => {
      ...
      for (const subdomain of reservedSubdomains) {
        const response = await request(app).get(`/api/check-subdomain/${subdomain}`);
    ...
    it('should handle special characters in URL', async () => {
      const response = await request(app).get('/api/check-subdomain/test%20company');
  ...
  describe('Multi-Tenant Isolation', () => {
    it('should ensure complete tenant isolation on signup', async () => {
      ...
      const [tenant1Rows] = await testDb.execute('SELECT id FROM tenants WHERE subdomain = ?', [
        'tenant1',
      ]);
      const tenant1 = asTestRows<unknown>(tenant1Rows);
      const [tenant2Rows] = await testDb.execute('SELECT id FROM tenants WHERE subdomain = ?', [
        'tenant2',
      ]);
      const tenant2 = asTestRows<unknown>(tenant2Rows);

      const [users1Rows] = await testDb.execute('SELECT * FROM users WHERE tenant_id = ?', [
        tenant1[0].id,
      ]);
      const users1 = asTestRows<unknown>(users1Rows);
      const [users2Rows] = await testDb.execute('SELECT * FROM users WHERE tenant_id = ?', [
        tenant2[0].id,
      ]);
  ...
  describe('Rate Limiting', () => {
    it('should rate limit signup attempts', async () => {
      ...
      const successCount = responses.slice(0, 5).filter((r) => r.status !== 429).length;
