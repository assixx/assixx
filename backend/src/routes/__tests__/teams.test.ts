...
describe('Team Management API Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    tenant2Id = await createTestTenant(testDb, 'teamtest2', 'Team Test Company 2');
    ...
    teamLeadToken = await getAuthToken(app, teamLeadUser.username, 'LeadPass123!');
  });
  ...
  describe('PUT /api/teams/:id', () => {
    ...
    it('should update team for admin', async () => {
      ...
      // Verify update
      const [rows] = await testDb.execute('SELECT * FROM teams WHERE id = ?', [updateTeamId]);
      const teams = asTestRows<unknown>(rows);
    ...
    it('should handle status changes', async () => {
      ...
      const [rows] = await testDb.execute('SELECT status FROM teams WHERE id = ?', [updateTeamId]);
      const teams = asTestRows<unknown>(rows);
  ...
  describe('DELETE /api/teams/:id', () => {
    ...
    it('should soft delete team for admin', async () => {
      ...
      // Verify soft delete
      const [rows] = await testDb.execute('SELECT status, deleted_at FROM teams WHERE id = ?', [
        deleteTeamId,
      ]);
      const teams = asTestRows<unknown>(rows);
      expect(teams[0].status).toBe('inactive');
    ...
    it('should reassign members if requested', async () => {
      ...
      const [rows] = await testDb.execute('SELECT team_id FROM users WHERE id = ?', [member.id]);
    ...
    it('should handle hard delete for admin', async () => {
      ...
      const [rows] = await testDb.execute('SELECT * FROM teams WHERE id = ?', [deleteTeamId]);
      const teams = asTestRows<unknown>(rows);
  ...
  describe('Team Member Management', () => {
    ...
    it('should enforce max members limit', async () => {
      // Set low max_members
      await testDb.execute('UPDATE teams SET max_members = 1 WHERE id = ?', [memberTeamId]);
    ...
    it('should remove member from team', async () => {
      ...
      // Verify removal
      const [rows] = await testDb.execute('SELECT team_id FROM users WHERE id = ?', [
        employeeUser1.id,
      ]);
      const users = asTestRows<unknown>(rows);
