...
describe('Department Management API Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    tenant2Id = await createTestTenant(testDb, 'depttest2', 'Department Test Company 2');
  ...
  describe('GET /api/departments', () => {
    ...
    it('should sort departments', async () => {
      ...
      for (let i = 1; i < departments.length; i++) {
        expect(departments[i].name.localeCompare(departments[i - 1].name)).toBeGreaterThanOrEqual(
          0,
        );
      }
    });
  ...
  describe('POST /api/departments', () => {
    ...
    it('should validate parent department exists in same tenant', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.message).toContain('Übergeordnete Abteilung nicht gefunden');
    });
  ...
  describe('PUT /api/departments/:id', () => {
    ...
    it('should update department for admin', async () => {
      ...
      // Verify update
      const [rows] = await testDb.execute('SELECT * FROM departments WHERE id = ?', [updateDeptId]);
      const departments = asTestRows<unknown>(rows);
    ...
    it('should update department hierarchy', async () => {
      ...
      const [rows] = await testDb.execute('SELECT parent_id FROM departments WHERE id = ?', [
        updateDeptId,
      ]);
      const departments = asTestRows<unknown>(rows);
      expect(departments[0].parent_id).toBe(dept1Id);
    ...
    it('should handle status changes', async () => {
      ...
      const [rows] = await testDb.execute('SELECT status FROM departments WHERE id = ?', [
        updateDeptId,
      ]);
      const departments = asTestRows<unknown>(rows);
  ...
  describe('DELETE /api/departments/:id', () => {
    ...
    it('should handle hard delete for admin', async () => {
      ...
      const [rows] = await testDb.execute('SELECT * FROM departments WHERE id = ?', [deleteDeptId]);
      const departments = asTestRows<unknown>(rows);
