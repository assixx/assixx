...
describe("UsersService Integration Tests", () => {
  ...
  afterAll(async () => {
    ...
    await execute("DELETE FROM tenants WHERE company_name LIKE ?", [
      "__AUTOTEST__%",
    ]);
  });
  ...
  describe("createUser", () => {
    ...
    it("should throw error for duplicate email", async () => {
      ...
      await expect(
        usersService.createUser(userData, testTenantId),
  ...
  describe("getUserById", () => {
    ...
    it("should throw error when user not found", async () => {
      await expect(
        usersService.getUserById(99999, testTenantId),
      ).rejects.toThrow(ServiceError);
  ...
  describe("listUsers", () => {
    ...
    it("should filter by search term", async () => {
      ...
      expect(
        users.every(
          (u) =>
            u.username.includes("__AUTOTEST__") ||
            u.email.includes("__AUTOTEST__"),
        ),
      ).toBe(true);
    });
  });
