...
describe('UsersService Logic Tests', () => {
  ...
  describe('Error Code Constants', () => {
    it('should have proper error codes', () => {
      ...
      const notFoundError = new ServiceError('NOT_FOUND', 'User not found', 404);
      expect(notFoundError.code).toBe('NOT_FOUND');
      expect(notFoundError.statusCode).toBe(404);

      const conflictError = new ServiceError('CONFLICT', 'Email already exists', 409);
      expect(conflictError.code).toBe('CONFLICT');
      expect(conflictError.statusCode).toBe(409);

      const unauthorizedError = new ServiceError('UNAUTHORIZED', 'Invalid password', 401);
      expect(unauthorizedError.code).toBe('UNAUTHORIZED');
      expect(unauthorizedError.statusCode).toBe(401);

      const badRequestError = new ServiceError('BAD_REQUEST', 'Invalid request', 400);
      expect(badRequestError.code).toBe('BAD_REQUEST');
      expect(badRequestError.statusCode).toBe(400);

      const serverError = new ServiceError('SERVER_ERROR', 'Internal server error', 500);
  ...
  describe('Business Logic Validation', () => {
    ...
    it('should validate limit parameters', () => {
      ...
      const tooHighLimit = Math.min(100, Math.max(1, parseInt('200', 10) ?? 20));
      expect(tooHighLimit).toBe(100);

      const tooLowLimit = Math.min(100, Math.max(1, parseInt('0', 10) ?? 20));
      expect(tooLowLimit).toBe(20);

      const invalidLimit = Math.min(100, Math.max(1, parseInt('invalid', 10) ?? 20));
    ...
    it('should validate sort order', () => {
      ...
      const isValidOrder = (order: string) => validOrders.includes(order.toLowerCase());
