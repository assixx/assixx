...
describe('Users v2 API Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    tenant2Id = await createTestTenant(testDb, 'usersv2test2', 'Users v2 Test Company 2');
  ...
  describe('Multi-Tenant Isolation', () => {
    ...
    it('should not list users from other tenants', async () => {
      ...
      const tenant2AdminEmail = users.find((u: any) => u.email.includes('admin.tenant2'));
      expect(tenant2AdminEmail).toBeDefined();

      // Verify we don't see any tenant1 users
      const tenant1Users = users.filter(
        (u: any) =>
          u.email.includes('admin.v2@test.com') || u.email.includes('employee.v2@test.com'),
      );
