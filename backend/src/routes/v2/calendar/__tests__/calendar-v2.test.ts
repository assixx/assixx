...
describe('Calendar v2 API Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    tenant2Id = await createTestTenant(testDb, 'calendarv2test2', 'Calendar v2 Test Company 2');
  ...
  beforeEach(async () => {
    ...
    await testDb.execute('DELETE FROM calendar_events WHERE tenant_id IN (?, ?)', [
      tenant1Id,
      tenant2Id,
    ]);
  });
    ...
    beforeEach(async () => {
      ...
      await testDb.execute(
        ...
        [
          ...
          new Date(now.getTime() + 3600000).toISOString().slice(0, 19).replace('T', ' '),
          'meeting',
          'confirmed',
        ],
      );
      ...
      await testDb.execute(
        ...
        [
          ...
          new Date(tomorrow.getTime() + 7200000).toISOString().slice(0, 19).replace('T', ' '),
          'other',
          'confirmed',
        ],
      );
    ...
    beforeEach(async () => {
      const [result] = await testDb.execute(
        `INSERT INTO calendar_events 
         (tenant_id, user_id, title, description, start_date, end_date, type, status) 
         VALUES (?, ?, ?, ?, NOW(), DATE_ADD(NOW(), INTERVAL 1 HOUR), ?, ?)`,
        [tenant1Id, adminUser.id, 'Test Event', 'Test Description', 'meeting', 'confirmed'],
      );
      ...
      await testDb.execute(
        `INSERT INTO calendar_attendees (event_id, user_id, response_status) 
         VALUES (?, ?, ?), (?, ?, ?)`,
        [eventId, adminUser.id, 'accepted', eventId, employeeUser.id, 'pending'],
      );
    });
    ...
    beforeEach(async () => {
      const [result] = await testDb.execute(
        `INSERT INTO calendar_events 
         (tenant_id, user_id, title, start_date, end_date, type, status) 
         VALUES (?, ?, ?, NOW(), DATE_ADD(NOW(), INTERVAL 1 HOUR), ?, ?)`,
        [tenant1Id, adminUser.id, 'Meeting with Attendees', 'meeting', 'confirmed'],
      );
      eventId = (result as any).insertId;
      ...
      await testDb.execute(
        `INSERT INTO calendar_attendees (event_id, user_id, response_status) 
         VALUES (?, ?, ?), (?, ?, ?)`,
        [eventId, employeeUser.id, 'pending', eventId, employee2User.id, 'pending'],
      );
    });
  ...
  describe('Multi-Tenant Isolation', () => {
    ...
    beforeAll(async () => {
      ...
      try {
        ...
        const [tenantCheck] = await testDb.execute('SELECT id FROM tenants WHERE id = ?', [
          tenant2Id,
        ]);
