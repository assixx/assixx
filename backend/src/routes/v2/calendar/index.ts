...
const eventValidation = createValidation([
  ...
  body('attendeeIds').optional().isArray().withMessage('Attendee IDs must be an array'),
...
const updateEventValidation = createValidation([
  ...
  body('endTime')
    ...
    .withMessage('End time must be after start time'),
  body('allDay').optional().isBoolean(),
  body('orgLevel').optional().isIn(['company', 'department', 'team', 'personal']),
...
const listEventsValidation = createValidation([
  ...
  query('filter').optional().isIn(['all', 'company', 'department', 'team', 'personal']),
