    ...
    for (const [index, question] of questions.entries()) {
      ...
      if (
        ...
        if (!Array.isArray(question.options) || question.options.length < 2) {
          throw new Error(
            `Question ${index + 1}: Choice questions need at least 2 options`,
          );
        }
      }
      ...
      // Check orderPosition
      if (
        question.orderPosition !== undefined &&
        typeof question.orderPosition !== "number"
      ) {
        throw new Error(
          `Question ${index + 1}: orderPosition must be a number`,
        );
      }
    }
    return true;
  ...
  .custom((assignments) => {
    for (const [index, assignment] of assignments.entries()) {
      ...
      if (assignment.type === "department" && !assignment.departmentId) {
        throw new Error(
          `Assignment ${index + 1}: departmentId required for department type`,
        );
      }
      if (assignment.type === "team" && !assignment.teamId) {
        throw new Error(
          `Assignment ${index + 1}: teamId required for team type`,
        );
      }
      if (assignment.type === "individual" && !assignment.userId) {
        throw new Error(
          `Assignment ${index + 1}: userId required for individual type`,
        );
      }
    }
    return true;
