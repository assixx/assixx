...
describe('Departments v2 API Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    tenant2Id = await createTestTenant(testDb, 'deptsv2test2', 'Departments v2 Test Company 2');
    ...
    if (adminLoginRes.status !== 200) {
      console.info('Admin login failed:', adminLoginRes.status, adminLoginRes.body);
    ...
    if (employeeLoginRes.status !== 200) {
      console.info('Employee login failed:', employeeLoginRes.status, employeeLoginRes.body);
    ...
    if (rootLoginRes.status !== 200) {
      console.info('Root login failed:', rootLoginRes.status, rootLoginRes.body);
    ...
    if (tenant2LoginRes.status !== 200) {
      console.info('Tenant2 login failed:', tenant2LoginRes.status, tenant2LoginRes.body);
  ...
  beforeEach(async () => {
    ...
    await testDb.query(`DELETE FROM departments WHERE tenant_id IN (?, ?) AND id NOT IN (?, ?)`, [
      tenant1Id,
      tenant2Id,
      dept1Id,
      dept2Id,
  ...
  describe('GET /api/v2/departments/stats', () => {
    ...
    it("should return stats only for user's tenant", async () => {
      ...
      await createTestDepartment(testDb, tenant2Id, `Tenant2 Test Dept ${String(Date.now())}`);
  ...
  describe('DELETE /api/v2/departments/:id', () => {
    ...
    it('should not delete department with assigned users', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.error.code).toBe('DEPT_400');
      expect(response.body.error.message).toContain('Cannot delete department with assigned users');

      // Clean up
      await testDb.query('UPDATE users SET department_id = NULL WHERE id = ?', [employeeUser.id]);
    });
  ...
  describe('GET /api/v2/departments/:id/members', () => {
    it('should return department members', async () => {
      ...
      await testDb.query('UPDATE users SET department_id = ? WHERE id IN (?, ?)', [
        dept1Id,
        adminUser.id,
        employeeUser.id,
      ]);
      ...
      // Clean up
      await testDb.query('UPDATE users SET department_id = NULL WHERE id IN (?, ?)', [
        adminUser.id,
        employeeUser.id,
      ]);
    });
