...
// Constants
const ADMIN_ID_ERROR = 'Admin ID must be a positive integer';

// Get admin permissions validation
export const getAdminPermissionsValidation: ValidationChain[] = [
  param('adminId').isInt({ min: 1 }).withMessage(ADMIN_ID_ERROR),
];
...
export const setPermissionsValidation: ValidationChain[] = [
  body('adminId').isInt({ min: 1 }).withMessage(ADMIN_ID_ERROR),
