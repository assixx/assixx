...
describe('Features API v2', () => {
  ...
  beforeAll(async () => {
    ...
    // Create test tenants
    tenantId1 = await createTestTenant(testDb, 'features-test-1', 'Test Features Tenant 1');

    tenantId2 = await createTestTenant(testDb, 'features-test-2', 'Test Features Tenant 2');
  ...
  afterAll(async () => {
    // Clean up test data
    if (testDb && tenantId1) {
      await testDb.execute('DELETE FROM feature_usage_logs WHERE tenant_id IN (?, ?)', [
        tenantId1,
        tenantId2,
      ]);
      await testDb.execute('DELETE FROM tenant_features WHERE tenant_id IN (?, ?)', [
        tenantId1,
        tenantId2,
      ]);
      await testDb.execute("DELETE FROM features WHERE code = 'test_feature_v2'");
    }
    await cleanupTestData();
  ...
  describe('GET /api/v2/features', () => {
    ...
    it('should respect includeInactive parameter', async () => {
      ...
      await testDb.execute("UPDATE features SET is_active = 0 WHERE code = 'test_feature_v2'");
      ...
      await testDb.execute("UPDATE features SET is_active = 1 WHERE code = 'test_feature_v2'");
    });
  });

  describe('GET /api/v2/features/categories', () => {
    it('should return features grouped by category', async () => {
      const response = await request(app).get('/api/v2/features/categories').expect(200);
      ...
      const premiumCategory = categories.find((c: any) => c.category === 'premium');
  ...
  describe('POST /api/v2/features/deactivate', () => {
    ...
    it('should return 404 if feature not activated', async () => {
      // Clean up first
      await testDb.execute('DELETE FROM tenant_features WHERE tenant_id = ?', [tenantId1]);
  ...
  describe('GET /api/v2/features/tenant/:tenantId', () => {
    ...
    it('should require authentication', async () => {
      await request(app).get(`/api/v2/features/tenant/${tenantId1}`).expect(401);
    });
  ...
  describe('GET /api/v2/features/all-tenants', () => {
    ...
    it('should return all tenants with feature summary', async () => {
      ...
      await testDb.execute(
        `INSERT INTO tenant_features (tenant_id, feature_id, is_active, activated_by) 
         VALUES (?, ?, 1, ?), (?, ?, 1, ?)`,
        [tenantId1, testFeatureId, adminUserId, tenantId2, testFeatureId, rootUserId],
      );
