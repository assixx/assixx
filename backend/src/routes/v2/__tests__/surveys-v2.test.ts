...
describe('Surveys API v2', () => {
  ...
  beforeAll(async () => {
    ...
    // Create test tenant
    tenantId = await createTestTenant(testDb, 'surveys-test', 'Test Surveys Tenant');
  ...
  beforeEach(async () => {
    ...
    await testDb.execute('DELETE FROM survey_responses WHERE tenant_id = ?', [tenantId]);
    await testDb.execute('DELETE FROM survey_answers WHERE tenant_id = ?', [tenantId]);
    await testDb.execute('DELETE FROM survey_assignments WHERE tenant_id = ?', [tenantId]);
    await testDb.execute('DELETE FROM survey_questions WHERE tenant_id = ?', [tenantId]);
    ...
    beforeEach(async () => {
      ...
      const [surveyResult] = await testDb.execute<ResultSetHeader>(
        `INSERT INTO surveys (tenant_id, title, description, status, is_anonymous, created_by)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [tenantId, 'All Users Survey', 'For all employees', 'active', 1, adminUserId],
      );
  ...
  describe('Survey Update and Delete Operations', () => {
    ...
    it('should delete survey without responses', async () => {
      ...
      // Verify deletion
      const [rows] = await testDb.execute('SELECT * FROM surveys WHERE id = ?', [testSurveyId]);
  ...
  describe('Multi-Tenant Isolation', () => {
    ...
    beforeEach(async () => {
      ...
      otherTenantId = await createTestTenant(testDb, 'other-survey-tenant', 'Other Tenant');
