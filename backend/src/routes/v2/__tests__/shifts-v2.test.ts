...
describe('Shifts API v2', () => {
  ...
  beforeAll(async () => {
    ...
    // Create test tenant
    tenantId = await createTestTenant(testDb, 'shifts-test', 'Test Shifts Tenant');
  ...
  beforeEach(async () => {
    ...
    await testDb.execute('DELETE FROM shift_swap_requests WHERE tenant_id = ?', [tenantId]);
    await testDb.execute('DELETE FROM shift_assignments WHERE tenant_id = ?', [tenantId]);
    await testDb.execute('DELETE FROM shifts WHERE tenant_id = ?', [tenantId]);
    await testDb.execute('DELETE FROM shift_templates WHERE tenant_id = ?', [tenantId]);
  ...
  describe('Shift Templates', () => {
    ...
    it('should list shift templates', async () => {
      ...
      await testDb.execute(
        ...
        [tenantId, 'Test Template', '2025-01-30 08:00:00', '2025-01-30 16:00:00', 30, '#3498db'],
      );
  ...
  describe('Multi-Tenant Isolation', () => {
    ...
    beforeEach(async () => {
      ...
      otherTenantId = await createTestTenant(testDb, 'other-shifts', 'Other Tenant');
    ...
    it('should not see templates from other tenant', async () => {
      ...
      await testDb.execute(
        'INSERT INTO shift_templates (tenant_id, name, start_time, end_time) VALUES (?, ?, ?, ?)',
        [tenantId, 'Original Tenant Template', '2025-01-30 08:00:00', '2025-01-30 16:00:00'],
      );
  ...
  describe('RootLog Integration', () => {
    ...
    it('should log swap request actions', async () => {
      ...
      await testDb.execute(
        ...
        [tenantId, shiftId, employeeUserId, 'assigned', 'accepted', adminUserId],
      );
