...
export const shiftsValidation = {
  ...
  listShifts: [
    ...
    query('userId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_USER_ID),
    query('departmentId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_DEPARTMENT_ID),
    query('teamId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_TEAM_ID),
    query('status')
      .optional()
    ...
    query('planId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_PLAN_ID),
  ...
  createShift: [
    ...
    body('departmentId').isInt({ min: 1 }).withMessage(MESSAGES.DEPARTMENT_ID_REQUIRED),
    body('planId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_PLAN_ID),
    body('templateId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_TEMPLATE_ID),
    body('title')
      .optional()
      .trim()
      .isLength({ max: 200 })
    ...
    body('teamId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_TEAM_ID),
    handleValidationErrors,
  ],

  updateShift: [
    param('id').isInt({ min: 1 }).withMessage(MESSAGES.SHIFT_ID_POSITIVE),
    body('userId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_USER_ID),
    ...
    body('departmentId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_DEPARTMENT_ID),
    body('planId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_PLAN_ID),
    body('templateId').optional().isInt({ min: 1 }).withMessage(MESSAGES.INVALID_TEMPLATE_ID),
    body('title')
      .optional()
  ...
  createTemplate: [
    ...
    body('isNightShift').optional().isBoolean().withMessage('Night shift flag must be a boolean'),
    body('isActive').optional().isBoolean().withMessage('Active flag must be a boolean'),
    handleValidationErrors,
  ],
  ...
  updateSwapRequestStatus: [
    param('id').isInt({ min: 1 }).withMessage('Swap request ID must be a positive integer'),
    body('status')
  ...
  getOvertimeReport: [
    ...
    query('endDate').notEmpty().isISO8601().withMessage('End date is required in ISO8601 format'),
    handleValidationErrors,
  ],
