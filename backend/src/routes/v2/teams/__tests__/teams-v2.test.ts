...
describe('Teams v2 API Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    tenant2Id = await createTestTenant(testDb, 'teamsv2test2', 'Teams v2 Test Company 2');
    ...
    dept1Id = await createTestDepartment(testDb, tenant1Id, 'Teams v2 Test Department');

    // Create test teams
    team1Id = await createTestTeam(testDb, tenant1Id, dept1Id, 'Teams v2 Test Team 1');

    team2Id = await createTestTeam(testDb, tenant1Id, dept1Id, 'Teams v2 Test Team 2');
    ...
    if (!adminLoginV2.body.success) {
      console.error('Admin login failed:', adminLoginV2.status, adminLoginV2.body);
  ...
  beforeEach(async () => {
    ...
    await testDb.execute('DELETE FROM user_teams WHERE team_id IN (?, ?)', [team1Id, team2Id]);
  });
  ...
  describe('DELETE /api/v2/teams/:id', () => {
    ...
    it('should prevent deletion of team with members', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.error.code).toBe('BAD_REQUEST');
      expect(response.body.error.message).toContain('Cannot delete team with members');
    });
