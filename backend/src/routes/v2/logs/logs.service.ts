...
export class LogsService {
  ...
  async getLogs(filters: LogsFilterParams): Promise<LogsListResponse> {
    ...
    try {
      ...
      logger.info(`[Logs v2 Service] Logs query params:`, [
        ...params,
        limit,
        offset,
      ]);

      const [logs] = await executeQuery<DbLogRow[]>(logsQuery, [
        ...params,
        limit,
        offset,
      ]);
      ...
      logger.error(
        "[Logs v2 Service] Error fetching logs - Detailed error:",
        error,
      );
      logger.error("[Logs v2 Service] Error stack:", (error as Error).stack);
      logger.error("[Logs v2 Service] Query params were:", {
        whereClause,
        params,
        limit,
        offset,
