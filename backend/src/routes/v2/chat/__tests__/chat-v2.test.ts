...
describe('Chat API v2', () => {
  ...
  afterAll(async () => {
    ...
    if (testDb && conversationId && groupConversationId) {
      ...
      await testDb.execute(`DELETE FROM messages WHERE conversation_id IN (?, ?)`, [
        conversationId,
        groupConversationId,
      ]);
      await testDb.execute(
    ...
    if (testDb && tenantId) {
      await testDb.execute(`DELETE FROM conversations WHERE tenant_id = ?`, [tenantId]);
    }
  ...
  describe('POST /api/v2/chat/conversations/:id/read', () => {
    it('should mark conversation as read', async () => {
      ...
      if (response.status !== 200) {
        ...
        process.stderr.write(`Response body: ${String(JSON.stringify(response.body))}\n`);
        process.stderr.write(`conversationId: ${conversationId}\n`);
        process.stderr.write(`URL: /api/v2/chat/conversations/${conversationId}/read\n`);
      }
