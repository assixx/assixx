...
describe('Blackboard API v2', () => {
  ...
  beforeAll(async () => {
    ...
    // Create test tenant
    tenantId = await createTestTenant(testDb, 'blackboard-test', 'Test Blackboard Tenant');
  });
  ...
  beforeEach(async () => {
    ...
    const [existingTenant] = await testDb.execute<any[]>('SELECT id FROM tenants WHERE id = ?', [
      tenantId,
    ]);
    ...
    await testDb.execute('DELETE FROM blackboard_entries WHERE tenant_id = ?', [tenantId]);
    await testDb.execute('DELETE FROM blackboard_tags WHERE tenant_id = ?', [tenantId]);
  ...
  describe('GET /api/v2/blackboard/entries', () => {
    it('should list all entries for authenticated user', async () => {
      ...
      const [teamInfo] = await testDb.execute<any[]>('SELECT * FROM user_teams WHERE user_id = ?', [
        employeeUserId,
      ]);
  ...
  describe('Multi-tenant isolation', () => {
    ...
    beforeEach(async () => {
      ...
      try {
        tenant2Id = await createTestTenant(testDb, 'blackboard-test2', 'Test Blackboard Tenant 2');
      } catch (error: unknown) {
        // Tenant might already exist, try to get its ID
        const [tenants] = await testDb.execute<any[]>('SELECT id FROM tenants WHERE code = ?', [
          'blackboard-test2',
        ]);
        if (tenants.length > 0) {
