...
const MACHINE_STATUS = ['operational', 'maintenance', 'repair', 'standby', 'decommissioned'];
...
export const machineValidation = {
  ...
  listMachines: [
    query('status').optional().isIn(MACHINE_STATUS).withMessage('Invalid status'),
    query('machineType').optional().isIn(MACHINE_TYPES).withMessage('Invalid machine type'),
    query('departmentId')
      .optional()
      .isInt({ min: 1 })
  ...
  machineId: [param('id').isInt({ min: 1 }).withMessage('Machine ID must be a positive integer')],
  ...
  createMachine: [
    ...
    body('machineType').optional().isIn(MACHINE_TYPES).withMessage('Invalid machine type'),
    body('status').optional().isIn(MACHINE_STATUS).withMessage('Invalid status'),
    body('purchaseDate')
      .optional()
      .isISO8601()
  ...
  updateMachine: [
    param('id').isInt({ min: 1 }).withMessage('Machine ID must be a positive integer'),
    body('name')
      .optional()
      .isString()
      .trim()
  ...
  addMaintenanceRecord: [
    ...
    body('statusAfter').optional().isIn(STATUS_AFTER).withMessage('Invalid status after'),
