...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        if (user && user.email) {
          ...
          if (hasEmailFeature) {
            ...
            const emailResult = await emailService.sendNewDocumentNotification(
              user,
              documentInfo
            );
            ...
            if (emailResult.success) {
              ...
              logger.info(
                `Email notification sent to user ${userId} for document ${documentId}`
              );
            } else {
              logger.warn(
                `Failed to send email notification to user ${userId}: ${emailResult.error}`
              );
            }
          } else {
            logger.info(
              `Email notifications feature not available for tenant ${tenantId}`
            );
          }
        }
      } catch (emailError) {
      ...
      logger.error(
        `Error uploading document by Admin ${adminId}: ${error.message}`
      );
      res.status(500).json({
...
router.get('/', authenticateToken, authorizeRole('admin'), async (req, res) => {
  ...
  logger.info(
    `Admin ${adminId} requesting all documents${category ? ` of category ${category}` : ''}`
  );
  ...
  try {
    ...
    logger.info(
      `Retrieved ${documents.length} documents${category ? ` of category ${category}` : ''}`
    );
    res.json(documents);
  } catch (error) {
...
router.get('/user/:userId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (req.user.role !== 'admin' && requesterId !== userId) {
      logger.warn(
        `Unauthorized access attempt: User ${requesterId} tried to access documents of user ${userId}`
      );
    ...
    logger.error(
      `Error retrieving documents for user ${userId}: ${error.message}`
    );
    res.status(500).json({
      message: 'Fehler beim Abrufen der Dokumente',
      error: error.message,
...
router.get(
  ...
  async (req, res) => {
    ...
    logger.info(
      `User ${requesterId} attempting to download document ${documentId}`
    );
    ...
    try {
      ...
      if (document.file_content && document.file_content.length > 1024 * 1024) {
        logger.info(
          `Streaming large document ${documentId} (size: ${document.file_content.length} bytes)`
        );
        ...
        for (let offset = 0; offset < fileSize; offset += CHUNK_SIZE) {
          const chunk = document.file_content.slice(
            offset,
      ...
      logger.info(
        `User ${requesterId} successfully downloaded document ${documentId}`
      );
    } catch (error) {
      logger.error(
        `Error downloading document ${documentId}: ${error.message}`
      );
      res.status(500).json({
...
router.put(
  ...
  async (req, res) => {
    ...
    logger.info(
      `Admin ${adminId} attempting to ${archived ? 'archive' : 'unarchive'} document ${documentId}`
    );
    ...
    try {
      ...
      if (success) {
        logger.info(
          `Document ${documentId} ${archived ? 'archived' : 'unarchived'} successfully by admin ${adminId}`
        );
        res.json({
        ...
        logger.warn(
          `Document ${documentId} not found for archiving/unarchiving`
        );
        res.status(404).json({ message: 'Dokument nicht gefunden' });
      }
    } catch (error) {
      logger.error(
        `Error ${archived ? 'archiving' : 'unarchiving'} document ${documentId}: ${error.message}`
      );
      res.status(500).json({
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (success) {
        logger.info(
          `Document ${documentId} deleted successfully by admin ${adminId}`
        );
