...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: 'Mitarbeiter erfolgreich erstellt', employeeId });
    } catch (error) {
      logger.error(
...
router.put(
  ...
  async (req, res) => {
    ...
    logger.info(
      `Admin ${adminId} attempting to toggle status of employee ${employeeId} to ${status}`
    );
    ...
    try {
      ...
      if (success) {
        logger.info(
          `Employee ${employeeId} status changed to ${status} by Admin ${adminId}`
        );
        res.json({
      ...
      logger.error(
        `Error updating status of employee ${employeeId} by Admin ${adminId}: ${error.message}`
      );
      res.status(500).json({
        message: 'Fehler beim Aktualisieren des Mitarbeiterstatus',
        error: error.message,
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: 'Dokument erfolgreich hochgeladen', documentId });
    } catch (error) {
      logger.error(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (employeeToDelete.role !== 'employee') {
        logger.warn(
          `User with ID ${employeeId} is not an employee but ${employeeToDelete.role}`
        );
        return res.status(403).json({
      ...
      try {
        ...
        if (documentCount > 0) {
          logger.warn(
            `Employee ${employeeId} has ${documentCount} documents, suggesting archive instead of delete`
          );
        ...
        logger.error(
          `Error checking for documents when deleting employee ${employeeId}: ${docError.message}`
        );
      ...
      if (success) {
        logger.info(
          `Employee with ID ${employeeId} deleted successfully by Admin ${adminId}`
        );
        res.json({
      ...
      logger.error(
        `Error deleting employee ${employeeId} by Admin ${adminId}: ${error.message}`
      );
      res.status(500).json({
        message: 'Fehler beim Löschen des Mitarbeiters',
        error: error.message,
...
router.post(
  '/upload-salary-document/:employeeId',
  authenticateToken,
  authorizeRole('admin'),
  upload.single('document'),
  async (req, res) => {
    ...
    logger.info(
      `Admin ${adminId} attempting to upload salary document for Employee ${employeeId}`
    );
    ...
    try {
      ...
      logger.info(
        `Admin ${adminId} successfully uploaded salary document ${documentId} for Employee ${employeeId}`
      );
      ...
      logger.error(
        `Error uploading salary document for Employee ${employeeId} by Admin ${adminId}: ${error.message}`
      );
      ...
      res.status(500).json({
        message: 'Fehler beim Hochladen der Gehaltsabrechnung',
        error: error.message,
      });
    }
  }
...
router.get(
  '/employee-salary-documents/:employeeId',
  authenticateToken,
  authorizeRole('admin'),
  async (req, res) => {
    const adminId = req.user.id;
    ...
    logger.info(
      `Admin ${adminId} requesting salary documents for Employee ${employeeId}`
    );
...
router.put(
  ...
  async (req, res) => {
    ...
    logger.info(
      `Admin ${adminId} attempting to archive document ${documentId}`
    );
    ...
    try {
      ...
      if (success) {
        logger.info(
          `Document ${documentId} archived successfully by Admin ${adminId}`
        );
        ...
        res
          .status(500)
          .json({ message: 'Fehler beim Archivieren des Dokuments' });
      }
    } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    ...
    logger.info(
      `Admin ${adminId} attempting to archive employee ${employeeId}`
    );
    ...
    try {
      ...
      if (employeeToArchive.role !== 'employee') {
        logger.warn(
          `User with ID ${employeeId} is not an employee but ${employeeToArchive.role}`
        );
        return res.status(403).json({
      ...
      if (success) {
        logger.info(
          `Employee with ID ${employeeId} archived successfully by Admin ${adminId}`
        );
        res.json({
      ...
      logger.error(
        `Error archiving employee ${employeeId} by Admin ${adminId}: ${error.message}`
      );
      res.status(500).json({
        message: 'Fehler beim Archivieren des Mitarbeiters',
        error: error.message,
...
router.post(
  ...
  async (req, res) => {
    ...
    logger.info(
      `Admin ${adminId} attempting to unarchive employee ${employeeId}`
    );
    ...
    try {
      ...
      if (success) {
        logger.info(
          `Employee with ID ${employeeId} unarchived successfully by Admin ${adminId}`
        );
        res.json({
      ...
      logger.error(
        `Error unarchiving employee ${employeeId} by Admin ${adminId}: ${error.message}`
      );
      res.status(500).json({
        message: 'Fehler beim Wiederherstellen des Mitarbeiters',
        error: error.message,
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      logger.info(
        `Retrieved ${archivedEmployees.length} archived employees for Admin ${adminId}`
      );

      res.json(archivedEmployees);
    } catch (error) {
      logger.error(
        `Error retrieving archived employees for Admin ${adminId}: ${error.message}`
      );
      res.status(500).json({
        message: 'Fehler beim Abrufen der archivierten Mitarbeiter',
        error: error.message,
      });
    }
  }
...
router.delete(
  ...
  async (req, res) => {
    ...
    logger.info(
      `Admin ${adminId} attempting to force-delete employee ${employeeId}`
    );
    ...
    try {
      ...
      if (success) {
        logger.info(
          `Employee with ID ${employeeId} force-deleted successfully by Admin ${adminId}`
        );
        res.json({
      ...
      logger.error(
        `Error force-deleting employee ${employeeId}: ${error.message}`
      );
      res.status(500).json({
