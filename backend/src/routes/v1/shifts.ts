...
// SQL Queries
const SQL_QUERIES = {
  GET_USER_DEPARTMENT: 'SELECT department_id FROM users WHERE id = ? AND tenant_id = ?',
} as const;
...
const availabilityValidation = createValidation([
  ...
  body('availability_type').notEmpty().withMessage('Verfügbarkeitstyp ist erforderlich'),
...
const exchangeRequestValidation = createValidation([
  body('original_shift_id').isInt({ min: 1 }).withMessage('Ungültige Schicht-ID'),
...
router.get(
  ...
  typed.auth(async (req, res) => {
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Laden der Schichtvorlagen', 500));
    }
  }),
);
  ...
  typed.body<ShiftTemplateBody>(async (req, res) => {
    try {
      ...
      if (!['admin', 'root', 'manager', 'team_lead'].includes(userRole)) {
        res
          .status(403)
          .json(errorResponse('Keine Berechtigung zum Erstellen von Schichtvorlagen', 403));
        return;
      }
      ...
      const templateData = {
        ...
        duration_hours: duration_hours > 0 ? duration_hours : 24 + duration_hours, // Handle overnight shifts
      };

      const template = await Shift.createShiftTemplate(templateData);
      res.status(201).json(successResponse(template, 'Schichtvorlage erfolgreich erstellt'));
    } catch (error: unknown) {
      console.error('Error creating shift template:', error);
      res
        .status(500)
        .json(
          errorResponse(getErrorMessage(error) || 'Fehler beim Erstellen der Schichtvorlage', 500),
        );
    }
  }),
);
...
router.get(
  ...
  typed.auth(async (req, res) => {
    try {
      const options: ShiftPlanFilters = {
        department_id:
          req.query.department_id != null && req.query.department_id !== '' ?
            Number.parseInt(
              ...
              : typeof req.query.department_id === 'number' ? String(req.query.department_id)
              : '0',
              10,
            )
          : undefined,
        team_id:
          req.query.team_id != null && req.query.team_id !== '' ?
            Number.parseInt(
              typeof req.query.team_id === 'string' ? req.query.team_id
              : typeof req.query.team_id === 'number' ? String(req.query.team_id)
              : '0',
              10,
            )
          : undefined,
        start_date:
          req.query.start_date != null && req.query.start_date !== '' ?
            ...
            : typeof req.query.start_date === 'object' ? JSON.stringify(req.query.start_date)
            : String(req.query.start_date)
          : undefined,
        end_date:
          req.query.end_date != null && req.query.end_date !== '' ?
            typeof req.query.end_date === 'string' ? req.query.end_date
            : typeof req.query.end_date === 'object' ? JSON.stringify(req.query.end_date)
            : String(req.query.end_date)
          : undefined,
        status:
          req.query.status != null && req.query.status !== '' ?
            ((typeof req.query.status === 'string' ? req.query.status
            : typeof req.query.status === 'object' ? JSON.stringify(req.query.status)
      ...
      res.status(500).json(errorResponse('Fehler beim Laden der Schichtpläne', 500));
    }
  }),
);
  ...
  typed.body<ShiftPlanBody>(async (req, res) => {
    try {
      ...
      if (!['admin', 'root', 'manager', 'team_lead'].includes(userRole)) {
        res
          .status(403)
          .json(errorResponse('Keine Berechtigung zum Erstellen von Schichtplänen', 403));
        return;
      }
      ...
      res.status(201).json(successResponse(plan, 'Schichtplan erfolgreich erstellt'));
    } catch (error: unknown) {
...
router.get(
  '/plans/:planId/shifts',
  ...security.user(
    createValidation([param('planId').isInt({ min: 1 }).withMessage('Ungültige Plan-ID')]),
  ),
  typed.params<{ planId: string }>(async (req, res) => {
    try {
      const planId = Number.parseInt(req.params.planId);
      const shifts = await Shift.getShiftsByPlan(planId, req.user.tenant_id, req.user.id);
...
router.get(
  ...
  ...security.user(
    createValidation([
      ...
      query('end').notEmpty().isISO8601().withMessage(VALIDATION_MESSAGES.ENDDATUM_ERFORDERLICH),
    ]),
  ),
  typed.auth(async (req, res) => {
    try {
      ...
      const startDate = new Date(
        ...
        : typeof start === 'object' ? JSON.stringify(start)
        : String(start),
      );
      const endDate = new Date(
        typeof end === 'string' ? end
        : typeof end === 'object' ? JSON.stringify(end)
        : String(end),
      );
      ...
      `;
        ...
        if (req.user.role === 'employee') {
          ...
          const [userRows] = await executeQuery<RowDataPacket[]>(SQL_QUERIES.GET_USER_DEPARTMENT, [
            req.user.id,
            tenantId,
          ]);
        ...
        const [rows] = await executeQuery<RowDataPacket[]>(sqlQuery, queryParams);
      ...
      console.error('Error fetching shifts:', error);
      res.status(500).json(errorResponse('Fehler beim Laden der Schichten', 500));
    }
  }),
);
...
router.get(
  ...
  ...security.user(
    createValidation([
      query('week').notEmpty().isISO8601().withMessage('Woche ist erforderlich'),
      query('department_id').optional().isInt({ min: 1 }),
    ]),
  ),
  typed.auth(async (req, res) => {
    try {
      ...
      const weekDate = new Date(
        ...
        : typeof week === 'object' ? JSON.stringify(week)
        : String(week),
      );
      ...
      try {
        ...
        if (req.user.role === 'employee') {
          ...
          departmentId = Number.parseInt(
            typeof department_id === 'string' ? department_id
            : typeof department_id === 'number' ? String(department_id)
            : '0',
            10,
          );
        }
        ...
        if (departmentId == null || departmentId === 0) {
          console.info('[SHIFTS NOTES] No department_id available, returning empty notes');
          res.json(
            successResponse({
              notes: '',
            }),
      ...
      `;
        ...
        if (rows.length > 0 && rows[0].notes != null) {
          ...
          if (Buffer.isBuffer(rows[0].notes)) {
            ...
            notes = Buffer.from((rows[0].notes as { data: number[] }).data).toString('utf8');
            console.info('[SHIFTS NOTES] Converted buffer object to string:', notes);
          } else {
            notes = rows[0].notes as string;
          }
        }

        console.info('[SHIFTS NOTES] Found notes:', notes ? `Yes: "${notes}"` : 'No');
...
router.post(
  ...
  typed.body<ShiftBody | WeeklyShiftBody>(async (req, res) => {
    try {
      ...
      const userRole = req.user.role;
      if (!['admin', 'root', 'manager', 'team_lead'].includes(userRole)) {
        res.status(403).json(errorResponse('Keine Berechtigung zum Erstellen von Schichten', 403));
        return;
      }

      const tenantId = req.user.tenant_id;
      const { week_start, week_end, assignments, notes } = req.body as WeeklyShiftBody;
      ...
      if (week_start !== '' && week_end !== '' && assignments.length > 0) {
        ...
        if (invalidAssignments.length > 0) {
          res
            .status(400)
            .json(errorResponse('Abteilung muss für alle Schichten ausgewählt werden', 400));
          return;
        }
        ...
        `;
          await connection.execute(deleteShiftsQuery, [tenantId, week_start, week_end]);
          ...
          for (const assignment of assignments) {
            ...
            const shiftTime = shiftTimes[assignment.shift_type as keyof typeof shiftTimes];
          ...
          if (
            ...
            await connection.execute(
             ...
             ON DUPLICATE KEY UPDATE notes = VALUES(notes), updated_at = NOW()`,
              [tenantId, assignments[0].department_id, week_start, notesString, req.user.id],
            );
          }
          ...
          res.json(successResponse(null, 'Schichtplan erfolgreich gespeichert'));
        } catch (error: unknown) {
          // Rollback on error
        ...
        res.status(201).json(successResponse(shift, 'Schicht erfolgreich erstellt'));
      }
    } catch (error: unknown) {
...
router.post(
  ...
  typed.paramsBody<{ shiftId: string }, ShiftAssignmentBody>(async (req, res) => {
    try {
      // Check if user has permission to assign shifts (admin, manager, team_lead)
      const userRole = req.user.role;
      if (!['admin', 'root', 'manager', 'team_lead'].includes(userRole)) {
        res.status(403).json(errorResponse('Keine Berechtigung zum Zuweisen von Schichten', 403));
        return;
      }
      ...
      res.status(201).json(successResponse(assignment, 'Mitarbeiter erfolgreich zugewiesen'));
    } catch (error: unknown) {
...
router.get(
  ...
  typed.auth(async (req, res) => {
    try {
      ...
      const targetUserId =
        user_id != null && user_id !== '' ? Number.parseInt(user_id as string) : req.user.id;
      ...
      if (targetUserId !== req.user.id) {
        ...
        if (!['admin', 'root', 'manager', 'team_lead'].includes(userRole)) {
          res
            .status(403)
            .json(errorResponse('Keine Berechtigung zum Anzeigen der Verfügbarkeit', 403));
          return;
        }
      }
      ...
      const availability = await Shift.getEmployeeAvailability(
        ...
        : typeof start_date === 'object' ? JSON.stringify(start_date)
        : String(start_date),
        typeof end_date === 'string' ? end_date
        : typeof end_date === 'object' ? JSON.stringify(end_date)
        : String(end_date),
      );
      ...
      res.status(500).json(errorResponse('Fehler beim Laden der Verfügbarkeit', 500));
    }
  }),
);
  ...
  typed.body<AvailabilityBody>(async (req, res) => {
    try {
      const availabilityData = {
        ...
        date: req.body.date,
        availability_type: req.body.availability_type as 'available' | 'unavailable' | 'partial',
      ...
      if (availabilityData.user_id !== req.user.id) {
        ...
        if (!['admin', 'root', 'manager', 'team_lead'].includes(userRole)) {
          res
            .status(403)
            .json(errorResponse('Keine Berechtigung zum Setzen der Verfügbarkeit', 403));
          return;
        }
      }

      const availability = await Shift.setEmployeeAvailability(availabilityData);
      res.json(successResponse(availability, 'Verfügbarkeit erfolgreich gesetzt'));
    } catch (error: unknown) {
...
router.get(
  ...
  typed.auth(async (req, res) => {
    try {
      const options: ShiftExchangeFilters = {
        status:
          req.query.status != null && req.query.status !== '' ?
            ((typeof req.query.status === 'string' ? req.query.status
            : typeof req.query.status === 'object' ? JSON.stringify(req.query.status)
            : String(req.query.status)) as 'pending' | 'approved' | 'rejected' | 'cancelled')
          : 'pending',
        limit: Number.parseInt(
          typeof req.query.limit === 'string' ? req.query.limit
          : req.query.limit != null ?
      ...
      res.status(500).json(errorResponse('Fehler beim Laden der Tauschbörse', 500));
    }
  }),
);
...
router.post(
  ...
  typed.body<ShiftExchangeBody>(async (req, res) => {
    try {
      ...
      res.status(201).json(successResponse(request, 'Tauschantrag erfolgreich erstellt'));
    } catch (error: unknown) {
...
router.get(
  ...
  typed.auth(async (req, res) => {
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Laden der eigenen Schichten', 500));
    }
  }),
);
...
router.get(
  ...
  typed.auth(async (req, res) => {
    try {
      ...
      const exchangeRequests = await Shift.getShiftExchangeRequests(tenantId, userId, {
        status: 'pending',
        limit: 5,
      });
      ...
      res.json(
        successResponse({
          ...
          stats: {
            ...
            availabilityDays: (availability as unknown as AvailabilityRecord[]).filter(
              (a) => a.availability_type === 'available',
            ).length,
          },
        }),
      );
      ...
      res.status(500).json(errorResponse('Fehler beim Laden des Schichtplan-Dashboards', 500));
    }
  }),
);
...
router.get(
  ...
  typed.auth(async (req, res) => {
    ...
    `;
      ...
      res.status(500).json(errorResponse('Fehler beim Laden der Wochenschichten', 500));
    }
  }),
);
...
router.get(
  ...
  typed.auth((_req, res) => {
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Laden der Wochennotizen', 500));
    }
  }),
);
...
router.post(
  ...
  ...security.user(
    createValidation([
      body('week').notEmpty().isISO8601().withMessage('Wochendatum ist erforderlich'),
      body('notes').optional().isString(),
      body('department_id').optional().isInt({ min: 1 }),
    ]),
  ),
  typed.body<WeeklyNotesBody>(async (req, res) => {
    try {
      ...
      await executeQuery(sqlQuery, [tenantId, departmentId, weekDate, notes ?? '', req.user.id]);

      res.json(successResponse(null, 'Notizen gespeichert'));
    } catch (error: unknown) {
      console.error('Error saving weekly notes:', error);
      res.status(500).json(errorResponse('Fehler beim Speichern der Wochennotizen', 500));
    }
  }),
);
...
router.post(
  ...
  typed.body<{ week: string; year: string; notes?: string }>(async (req, res) => {
    try {
      ...
      await executeQuery(notesQuery, [tenantId, weekStart, notes ?? '', req.user.id]);
