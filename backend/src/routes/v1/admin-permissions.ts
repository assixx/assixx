...
router.get(
  ...
  typed.params<{ adminId: string }>(async (req, res) => {
    ...
    try {
      ...
      const result = await adminPermissionService.getAdminDepartments(adminId, targetTenantId);

      res.json(successResponse(result));
    } catch (error: unknown) {
      logger.error('Error getting admin departments:', error);
      res.status(500).json(errorResponse('Fehler beim Abrufen der Abteilungsberechtigungen', 500));
    }
  }),
);
...
router.get(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Abrufen der eigenen Abteilungen', 500));
    }
  }),
);
...
router.post(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      if (success) {
        logger.info(`Root user ${req.user.id} set permissions for admin ${adminId}`);
        res.json(successResponse(null, 'Berechtigungen erfolgreich gesetzt'));
      } else {
        res.status(500).json(errorResponse('Fehler beim Setzen der Berechtigungen', 500));
      }
    } catch (error: unknown) {
...
router.post(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      if (success) {
        logger.info(`Root user ${req.user.id} set group permissions for admin ${adminId}`);
        res.json({
          success: true,
      ...
      logger.error(`Error setting admin group permissions: ${getErrorMessage(error)}`);
      res.status(500).json({
        success: false,
        error: 'Fehler beim Setzen der Gruppenberechtigungen',
      });
...
router.delete(
  ...
  typed.params<{ adminId: string; departmentId: string }>(async (req, res) => {
    ...
    try {
      ...
      logger.error(`Error removing admin permission: ${getErrorMessage(error)}`);
      res.status(500).json({
        success: false,
...
router.delete(
  ...
  typed.params<{ adminId: string; groupId: string }>(async (req, res) => {
    ...
    try {
      ...
      logger.error(`Error removing admin group permission: ${getErrorMessage(error)}`);
      res.status(500).json({
        success: false,
...
router.post(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      logger.error(`Error in bulk permission operation: ${getErrorMessage(error)}`);
      res.status(500).json({
        success: false,
