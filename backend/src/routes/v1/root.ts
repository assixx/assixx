...
router.post(
  ...
  typed.body<{
    ...
    try {
      ...
      logger.error('Fehler beim Erstellen des Admin-Benutzers:', getErrorMessage(error));
...
router.get(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      logger.error('Fehler beim Abrufen der Admin-Benutzer:', getErrorMessage(error));
      res.status(500).json({
...
router.put(
  ...
  typed.paramsBody<{ id: string }, AdminUpdateData>(async (req, res) => {
    ...
    logger.info(`Updating admin (ID: ${adminId}) by root user: ${req.user.username}`);
    logger.info(`Update data received:`, updateData);

    try {
      // Prüfen ob Admin existiert
      const admin = await User.findById(Number.parseInt(adminId, 10), req.user.tenant_id);
...
router.delete(
  '/admins/:id',
  ...security.root(
    createValidation([param('id').isInt({ min: 1 }).withMessage('Ungültige Admin-ID')]),
  ),
  typed.params<{ id: string }>(async (req, res) => {
    ...
    try {
      ...
      const adminToDelete = await User.findById(Number.parseInt(adminId, 10), req.user.tenant_id);
      ...
      if (adminToDelete.role !== 'admin') {
        ...
        res.status(403).json({ message: 'Der zu löschende Benutzer ist kein Admin' });
        return;
      }
      ...
      if (success) {
        ...
        res.status(500).json(errorResponse('Fehler beim Löschen des Admin-Benutzers', 500));
      }
    } catch (error: unknown) {
...
router.get(
  ...
  typed.params<{ id: string }>(async (req, res) => {
    ...
    try {
      ...
      if (admin.role !== 'admin') {
        logger.warn(`User with ID ${adminId} is not an admin`);
        res.status(403).json({ message: 'Der abgefragte Benutzer ist kein Admin' });
        return;
      }
      ...
      res.status(500).json(errorResponse('Fehler beim Abrufen der Admin-Details', 500));
    }
  }),
);
...
router.put(
  ...
  typed.paramsBody<{ id: string }, LegacyAdminUpdateData>(async (req, res) => {
    ...
    try {
      ...
      if (admin.role !== 'admin') {
        logger.warn(`User with ID ${adminId} is not an admin`);
        res.status(403).json({ message: 'Der zu aktualisierende Benutzer ist kein Admin' });
        return;
      }
      ...
      if (new_password != null && new_password !== '' && new_password.trim() !== '') {
        updateData.password = await bcrypt.hash(new_password, 10);
      }

      // Admin aktualisieren
      await User.update(Number.parseInt(adminId, 10), updateData, req.user.tenant_id);

      logger.info(`Admin ${adminId} updated successfully by root user ${rootUser}`);
      res.json(successResponse(null, 'Admin-Benutzer erfolgreich aktualisiert'));
    } catch (error: unknown) {
      logger.error(`Error updating admin ${adminId}:`, error);
      res.status(500).json(errorResponse('Fehler beim Aktualisieren des Admin-Benutzers', 500));
    }
  }),
);
...
router.get(
  ...
  typed.params<{ id: string }>(async (req, res) => {
    ...
    try {
      ...
      // Logs abrufen
      const logs = await getRootLogsByUserId(Number.parseInt(adminId, 10), days);

      logger.info(`Retrieved ${logs.length} logs for admin ${adminId}`);
      res.json(successResponse(logs));
    } catch (error: unknown) {
      logger.error(`Error retrieving logs for admin ${adminId}:`, error);
      res.status(500).json(errorResponse('Fehler beim Abrufen der Admin-Logs', 500));
    }
  }),
);
...
router.get(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Abrufen der Tenants', 500));
    }
  }),
);
...
router.get(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Abrufen der Dashboard-Daten', 500));
    }
  }),
);
...
router.get(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Abrufen der Speicherinformationen', 500));
    }
  }),
);
...
router.get(
  ...
  typed.auth(async (_req, res) => {
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Abrufen des Löschstatus'));
    }
  }),
);
        ...
        successResponse(
          {
            ...
            message: 'Löschung beantragt - Genehmigung durch zweiten Root-User erforderlich',
...
router.post(
  ...
  typed.paramsBody<{ id: string }, { reason?: string }>(async (req, res) => {
    try {
      ...
      await tenantDeletionService.rejectDeletion(queueId, approverId, reason ?? 'Keine Angabe');

      res.json(successResponse({ message: 'Löschung abgelehnt' }));
    } catch (error: unknown) {
      logger.error('Error rejecting deletion:', error);
      const message = error instanceof Error ? error.message : 'Fehler beim Ablehnen';
      res.status(400).json(errorResponse(message));
    }
  }),
);
...
router.post(
  ...
  typed.params<{ id: string }>(async (req, res) => {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Fehler beim Emergency Stop';
    ...
    try {
      ...
      if (rootUser.tenant_id !== tenantId) {
        ...
        res
          .status(403)
          .json(
            errorResponse('ZUGRIFF VERWEIGERT: Sie können nur Ihren eigenen Tenant löschen', 403),
          );
        return;
      }
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      if (deletionQueue.length === 0) {
        res.status(404).json(errorResponse('Keine aktive Löschung gefunden', 404));
        return;
      }
      ...
      res.status(500).json(errorResponse('Fehler beim Abrufen des Löschstatus', 500));
    }
  }),
);
  ...
  typed.params<{ id: string }>(async (req, res) => {
    ...
    try {
      ...
      if (rootUser.tenant_id !== tenantId) {
        res
          .status(403)
          .json(errorResponse('Sie können nur Ihren eigenen Tenant-Status einsehen', 403));
        return;
      }
      ...
      if (!status) {
        res.status(404).json(errorResponse('Keine Löschung für diesen Tenant geplant', 404));
        return;
      }

      res.json(successResponse(status));
    } catch (error: unknown) {
      logger.error(`Error getting deletion status for tenant ${tenantId}:`, error);
      res.status(500).json(errorResponse('Fehler beim Abrufen des Löschstatus', 500));
    }
  }),
);
...
router.post(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      res.json(successResponse({ tenantId }, 'Löschung erfolgreich abgebrochen'));
    } catch (error: unknown) {
...
router.post(
  ...
  typed.params<{ id: string }>(async (req, res) => {
    ...
    try {
      ...
      if (rootUser.tenant_id !== tenantId) {
        res.status(403).json(errorResponse('Sie können nur Ihre eigene Löschung abbrechen', 403));
        return;
      }

      await tenantDeletionService.cancelDeletion(tenantId, rootUser.id);

      res.json(successResponse({ cancelled: true }, 'Löschung wurde erfolgreich abgebrochen'));
    } catch (error: unknown) {
...
router.post(
  ...
  ...security.root(),
  typed.paramsBody<{ queueId: string }, { comment?: string }>(async (req, res) => {
    const queueId = Number.parseInt(req.params.queueId, 10);
    const rootUser = req.user;

    logger.info(`Root user ${rootUser.username} approving deletion request ${queueId}`);

    try {
      await tenantDeletionService.approveDeletion(queueId, rootUser.id, req.body.comment);
...
router.post(
  ...
  ...security.root(),
  typed.paramsBody<{ queueId: string }, { reason: string }>(async (req, res) => {
    const queueId = Number.parseInt(req.params.queueId, 10);
    const rootUser = req.user;

    logger.info(`Root user ${rootUser.username} rejecting deletion request ${queueId}`);

    try {
      if (!req.body.reason) {
        res.status(400).json(errorResponse('Grund für Ablehnung ist erforderlich', 400));
        return;
      }

      await tenantDeletionService.rejectDeletion(queueId, rootUser.id, req.body.reason);
...
router.get(
  ...
  typed.auth(async (req, res) => {
    ...
    logger.info(`Root user ${rootUser.username} requesting pending deletion approvals`);
    ...
    try {
      ...
      res
        .status(500)
        .json(errorResponse('Fehler beim Abrufen der ausstehenden Genehmigungen', 500));
    }
  }),
);
...
router.post(
  ...
  typed.params<{ id: string }>(async (req, res) => {
    ...
    logger.info(`Root user ${rootUser.username} requesting dry-run for tenant ${tenantId}`);

    try {
      if (rootUser.tenant_id !== tenantId) {
        res.status(403).json(errorResponse('Sie können nur Ihren eigenen Tenant simulieren', 403));
        return;
      }
...
router.post(
  ...
  typed.params<{ queueId: string }>(async (req, res) => {
    ...
    logger.warn(`Root user ${rootUser.username} retrying failed deletion ${queueId}`);

    try {
      await tenantDeletionService.retryDeletion(queueId);

      res.json(successResponse({ retrying: true }, 'Löschung wird erneut versucht'));
    } catch (error: unknown) {
...
router.get(
  ...
  typed.auth(async (req, res) => {
    ...
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Laden der Root-Benutzer', 500));
    }
  }),
);
...
router.get(
  ...
  typed.params<{ id: string }>(async (req, res) => {
    ...
    try {
      ...
      if (users.length === 0) {
        res.status(404).json(errorResponse('Root-Benutzer nicht gefunden', 404));
        return;
      }
      ...
      res.status(500).json(errorResponse('Fehler beim Laden des Root-Benutzers', 500));
    }
  }),
);
...
router.post(
  ...
  typed.body<{
    ...
    logger.warn(`Root user ${rootUser.username} creating new root user: ${email}`);
    ...
    try {
      ...
      if (existing.length > 0) {
        res.status(400).json(errorResponse('E-Mail-Adresse wird bereits verwendet', 400));
        return;
      }
      ...
      const subdomain: string = (tenantData[0]?.subdomain as string) || 'DEFAULT';
      ...
      logger.info(`Root user created successfully: ${email} (ID: ${result.insertId})`);

      res.json(successResponse({ id: result.insertId }, 'Root-Benutzer erfolgreich erstellt'));
    } catch (error: unknown) {
      logger.error('Error creating root user:', error);
      res.status(500).json(errorResponse('Fehler beim Erstellen des Root-Benutzers', 500));
    }
  }),
);
...
router.put(
  ...
  >(async (req, res) => {
    const userId = Number.parseInt(req.params.id, 10);
    const rootUser = req.user;
    const { first_name, last_name, email, position, notes, is_active } = req.body;
    ...
    try {
      ...
      res.status(500).json(errorResponse('Fehler beim Aktualisieren des Root-Benutzers', 500));
    }
  }),
);
  ...
  typed.params<{ id: string }>(async (req, res) => {
    ...
    try {
      ...
      if (rootCount[0].count < 1) {
        res
          .status(400)
          .json(errorResponse('Es muss mindestens ein Root-Benutzer im System verbleiben', 400));
        return;
      }
      ...
      res.status(500).json(errorResponse('Fehler beim Löschen des Root-Benutzers', 500));
    }
  }),
);
