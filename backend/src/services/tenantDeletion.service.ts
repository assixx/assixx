...
export class TenantDeletionService {
  ...
  private readonly executor = new TenantDeletionExecutor();
  ...
  public async deleteTenant(
    ...
    return await transaction(async (conn: PoolConnection) => {
      ...
      try {
        await this.audit.checkLegalHolds(tenantId, wrappedConn);
        ...
        await this.analyzer.verifyCompleteDeletion(tenantId, wrappedConn);
  ...
  async requestDeletion(
    ...
    // Check legal holds
    const [legalHolds] = await query<LegalHoldRow[]>(
