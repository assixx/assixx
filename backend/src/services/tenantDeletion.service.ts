...
export class TenantDeletionService {
    ...
    this.steps = [
      ...
      },
      {
        ...
        handler: async (tenantId: number, _queueId: number, conn: ConnectionWrapper) => {
          try {
            ...
            const [tenantInfo] = await conn.query(
          ...
          for (const admin of admins) {
            await emailService.sendEmail({
              ...
              html: `
                ...
                <p>Sehr geehrte/r ${(admin as any).first_name} ${(admin as any).last_name},</p>
        ...
        handler: async (tenantId: number, _queueId: number, conn: ConnectionWrapper) => {
          ...
          for (const webhook of webhooks) {
            try {
              ...
              logger.warn(`Webhook notification failed: ${webhook.url}`, error);
            }
          }
      ...
      // ========== CACHE & SESSION CLEANUP ==========
      {
        ...
        handler: async (tenantId: number, _queueId: number, conn: ConnectionWrapper) => {
          ...
          try {
            ...
            for (const user of userIds as any[]) {
              // Lösche alle Keys die mit user:id: beginnen
              const keys = await redis.keys(`user:${(user as any).id}:*`);
              if (keys.length > 0) {
      ...
      },
      {
        ...
        handler: async (tenantId: number, queueId: number, conn: ConnectionWrapper) => {
          ...
          if (!queueResult || !queueResult[0] || !queueResult[0].created_by) {
          ...
          const result = await conn.query(
            ...
            [tenantId, queueResult[0].created_by]
          );
          return result.affectedRows || 0;
        },
      },
      ...
      // ========== POST-DELETION PHASE ==========
      {
        ...
        critical: false,
        handler: async (tenantId: number, _queueId: number, _conn: ConnectionWrapper) => {
  ...
  async requestTenantDeletion(
    ...
    return await transaction(async (connection) => {
      ...
      const currentStatus = (tenant as any).deletion_status || 'active';
      ...
      const coolingOffHours = process.env.DELETION_COOLING_OFF_HOURS 
        ...
        : (process.env.NODE_ENV === 'development' ? 0 : 24);
  ...
  async approveDeletion(
    ...
    return await transaction(async (connection) => {
      // Get queue item
      const [queueResults] = await connection.query<RowDataPacket[]>(
      ...
      const coolingOffHours = queue.cooling_off_hours !== undefined && queue.cooling_off_hours !== null 
        ? queue.cooling_off_hours 
        : 24;
      ...
      await connection.query(
        ...
        ['suspended', (queue as any).tenant_id]
      );
      ...
      await this.notifyApprovalStatus(
        (queue as any).tenant_id,
        (queue as any).created_by,
        'approved',
        approverId
      );
      ...
      await alertingService
        .sendSlackAlert({
          ...
          fields: {
            'Queue ID': queueId,
            'Tenant ID': (queue as any).tenant_id,
            'Approved By': `User ${approverId}`,
            'Scheduled Date': new Date(
              (queue as any).scheduled_deletion_date
            ).toISOString(),
          },
        })
  ...
  async rejectDeletion(
    ...
    await transaction(async (connection) => {
      ...
      if ((queue as any).approval_status !== 'pending') {
        throw new Error('Deletion request is not pending approval');
      }
  ...
  async emergencyStop(queueId: number, stoppedBy: number): Promise<void> {
    try {
      ...
      if (queueItem) {
        ...
        logger.warn(
          `EMERGENCY STOP activated for tenant ${(queueItem as any).tenant_id} deletion by user ${stoppedBy}`
        );
      }
    } catch (error) {
  ...
  private async processTenantDeletion(queueId: number): Promise<void> {
    ...
    try {
      ...
      for (const step of this.steps) {
        ...
        if (emergencyCheck && (emergencyCheck as any).emergency_stop) {
        ...
        try {
          ...
          try {
            recordsDeleted = await transaction(async (stepConnection) => {
              ...
              // Create a wrapper that handles the connection query format
              const connWrapper: ConnectionWrapper = {
                query: async (sql: string, params?: unknown[]) => {
                  const [result] = await stepConnection.query(sql, params);
                  return result;
                },
                execute: async (sql: string, params?: unknown[]) => {
                  const [result] = await stepConnection.execute(sql, params);
                  return result;
                }
              };
  ...
  private async handleEmergencyStop(
    ...
    // Notify admins
    const [stoppedByUser] = await query(
      'SELECT emergency_stopped_by FROM tenant_deletion_queue WHERE id = ?',
      [queueId]
    );

    if (stoppedByUser && (stoppedByUser as any).emergency_stopped_by) {
      await this.notifyEmergencyStop(
        tenantId,
        queueId,
        (stoppedByUser as any).emergency_stopped_by
      );
    }
  }
  ...
  private async notifyRootAdminsForApproval(
    ...
    const rootUsers = await query(
      'SELECT * FROM users WHERE role = "root" AND id != ?',
      [requestedBy]
    );

    const [requestedByUser] = await query(
    ...
    for (const root of rootUsers) {
      await emailService.sendEmail({
        to: (root as any).email,
        subject: 'Genehmigung erforderlich: Tenant-Löschung',
        html: `
          <h2>Tenant-Löschung Genehmigung erforderlich</h2>
          <p><strong>${(requestedByUser as any).first_name} ${(requestedByUser as any).last_name}</strong> (${(requestedByUser as any).username}) hat die Löschung des folgenden Tenants beantragt:</p>
          <ul>
  ...
  private async notifyApprovalStatus(
    ...
    const [requester] = await query('SELECT * FROM users WHERE id = ?', [
      requesterId,
    ]);
    const [approver] = await query('SELECT username FROM users WHERE id = ?', [
      approverId,
    ]);
    ...
    const statusText =
      status === 'approved'
        ? `<p style="color: green;"><strong>✅ Genehmigt von ${(approver as any).username}</strong></p>`
        : `<p style="color: red;"><strong>❌ Abgelehnt von ${(approver as any).username}</strong></p>`;
    ...
    await emailService.sendEmail({
      ...
      html: `
        <h2>${subject}</h2>
        <p>Ihre Anfrage zur Löschung des Tenants <strong>${(tenant as any).company_name}</strong> wurde bearbeitet.</p>
        ${statusText}
        ...
        `
            : `
          <p>Der Tenant bleibt aktiv. Bei Fragen wenden Sie sich bitte an ${(approver as any).username}.</p>
        `
        }
      `,
    });
  ...
  private async _notifyRootAdmins(tenantId: number, deletedBy: number, tenantName: string): Promise<void> {
    ...
    const [deletedByUser] = await query(
    ...
    for (const root of rootUsers) {
      await emailService.sendEmail({
        ...
        html: `
          ...
          <p>Gelöscht von: ${(deletedByUser as any).username}</p>
    ...
    for (const root of rootUsers) {
      await emailService.sendEmail({
        ...
        html: `
          ...
          <p>Die Löschung des Tenants <strong>${(tenant as any).company_name}</strong> wurde durch einen Emergency Stop angehalten.</p>
          <p><strong>Gestoppt von:</strong> ${(user as any).username}</p>
          <p><strong>Queue ID:</strong> ${queueId}</p>
   ...
   */
  async performDryRun(tenantId: number): Promise<any> {
    const report = {
      tenantId,
      estimatedDuration: 0,
    ...
    // Check for blockers
    const [legalHold] = await query(
      'SELECT * FROM legal_holds WHERE tenant_id = ? AND active = 1',
      [tenantId]
    );

    if (legalHold) {
      report.blockers.push(`Legal hold active: ${(legalHold as any).reason}`);
    }
    ...
    for (const step of this.steps) {
      try {
        ...
        report.warnings.push(
          `Could not estimate ${step.name}: ${error.message}`
        );
      }
    }
