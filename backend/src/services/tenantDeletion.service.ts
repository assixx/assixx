...
interface ConnectionWrapper {
  query: <T = RowDataPacket[] | ResultSetHeader>(
    sql: string,
    params?: unknown[]
  ) => Promise<[T, unknown]>;
  execute?: <T = RowDataPacket[] | ResultSetHeader>(
    sql: string,
    params?: unknown[]
  ) => Promise<[T, unknown]>;
}
...
export class TenantDeletionService {
    ...
    this.steps = [
      ...
      // Level 20: Scheduled Tasks & Cronjobs
      {
        ...
        handler: async (
          ...
          // Scheduled Tasks
          const tasksResult = await conn.query(
            'DELETE FROM scheduled_tasks WHERE tenant_id = ?',
            [tenantId]
          );
  ...
  async approveDeletion(
    ...
    return await transaction(async (connection) => {
      ...
      const coolingOffHours =
        queue.cooling_off_hours !== undefined &&
        queue.cooling_off_hours !== null
  ...
  async emergencyStop(queueId: number, stoppedBy: number): Promise<void> {
    try {
      ...
      if (queueItem) {
        ...
        await this.notifyEmergencyStop(queueItem.tenant_id, queueId, stoppedBy);

        // Log the emergency stop
        logger.warn(
          `EMERGENCY STOP activated for tenant ${queueItem.tenant_id} deletion by user ${stoppedBy}`
        );
      }
    } catch (error) {
  ...
  private async processTenantDeletion(queueId: number): Promise<void> {
    ...
    try {
      ...
      for (const step of this.steps) {
        ...
        if (emergencyCheck && emergencyCheck.emergency_stop) {
  ...
  private async handleEmergencyStop(
    ...
    if (stoppedByUser && stoppedByUser.emergency_stopped_by) {
   ...
   * Get deletion status
   */
  async getDeletionStatus(tenantId: number): Promise<RowDataPacket | null> {
    ...
    for (const step of this.steps) {
      try {
        ...
        if (step.name.includes('users')) {
          ...
          const countResult = result as CountResult;
