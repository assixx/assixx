...
function generateTestToken(userId: number, tenantId: number, role: string = 'root'): string {
  return jwt.sign(
...
async function cleanupTestData(tenantId: number): Promise<void> {
  try {
    ...
    await query('DELETE FROM tenant_deletion_log WHERE queue_id IN (SELECT id FROM tenant_deletion_queue WHERE tenant_id = ?)', [tenantId]);
    await query('DELETE FROM deletion_audit_trail WHERE tenant_id = ?', [tenantId]);
    await query('DELETE FROM tenant_deletion_queue WHERE tenant_id = ?', [tenantId]);
    await query('DELETE FROM tenant_data_exports WHERE tenant_id = ?', [tenantId]);
    await query('UPDATE tenants SET deletion_status = "active", deletion_requested_at = NULL WHERE id = ?', [tenantId]);
  } catch (error) {
...
describe('Tenant Deletion Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const [userResult] = await query(
      ...
      [testTenantId, 'test-root', 'root@test-deletion.com', 'hashed_password', 'root']
    );
  ...
  describe('DELETE /api/root/tenants/:id', () => {
    it('should successfully initiate tenant deletion', async () => {
      ...
      const [tenant] = await query('SELECT deletion_status FROM tenants WHERE id = ?', [testTenantId]);
      expect(tenant.deletion_status).toBe('marked_for_deletion');

      const [queueEntry] = await query('SELECT * FROM tenant_deletion_queue WHERE tenant_id = ?', [testTenantId]);
  ...
  describe('GET /api/root/tenants/:id/deletion-status', () => {
    beforeEach(async () => {
      ...
      await tenantDeletionService.markTenantForDeletion(testTenantId, testUserId, 'Status test');
    });
  ...
  describe('POST /api/root/tenants/:id/cancel-deletion', () => {
    beforeEach(async () => {
      ...
      await tenantDeletionService.markTenantForDeletion(testTenantId, testUserId, 'Cancel test');
    });
  ...
  describe('Tenant Status Middleware', () => {
    it('should block access to suspended tenant', async () => {
      ...
      await query('UPDATE tenants SET deletion_status = ? WHERE id = ?', ['suspended', testTenantId]);
    ...
    it('should allow whitelisted routes during deletion', async () => {
      await query('UPDATE tenants SET deletion_status = ? WHERE id = ?', ['marked_for_deletion', testTenantId]);
  ...
  describe('Complete Deletion Flow', () => {
    it('should process deletion after grace period', async () => {
      ...
      const [tenant] = await query('SELECT * FROM tenants WHERE id = ?', [testTenantId]);
  ...
  describe('Error Recovery', () => {
    it('should retry failed deletion', async () => {
      ...
      const [queueEntry] = await query('SELECT status, retry_count FROM tenant_deletion_queue WHERE id = ?', [queueId]);
      expect(queueEntry.status).toBe('queued');
    ...
    it('should handle partial deletion failures', async () => {
      ...
      jest.spyOn(tenantDeletionService as any, 'executeStep')
