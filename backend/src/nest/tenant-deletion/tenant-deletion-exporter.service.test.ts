/**
 * Unit tests for TenantDeletionExporter (NestJS version)
 *
 * Migrated from services/tenant-deletion-exporter.service.test.ts
 * Key changes:
 * - Removed vi.mock('../utils/logger.js') — NestJS Logger is internal, no mock needed
 * - client.query() returns { rows: T[] } (PoolClient) instead of T[] (ConnectionWrapper)
 * - INSERT queries return { rows: [], rowCount: 1 }
 * - Import path updated to local ./tenant-deletion-exporter.service.js
 * - Helper import updated to local ./tenant-deletion.helpers.js
 */
...
describe('TenantDeletionExporter', () => {
  ...
  describe('fetchTenantInfo', () => {
    it('should return tenant info from DB', async () => {
      ...
      const result = await priv.fetchTenantInfo(1, mockClient);
    ...
    it('should return undefined when tenant not found', async () => {
      ...
      const result = await priv.fetchTenantInfo(999, mockClient);
  ...
  describe('createTenantDataExport', () => {
    it('should orchestrate full backup creation', async () => {
      ...
      // fetchTenantInfo — PoolClient returns { rows: [...] }
    ...
    it('should handle table export errors gracefully', async () => {
      ...
      mockClient.query.mockRejectedValueOnce(new Error('table gone'));
