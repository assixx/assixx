 ...
 * PoolClient for transaction-scoped work.
 * NOTE: emailService remains as direct import (Phase 1 — separate migration ticket).
 */
...
export class TenantDeletionAudit {
  private readonly logger = new Logger(TenantDeletionAudit.name);
   ...
   */
  async checkLegalHolds(tenantId: number, client: PoolClient): Promise<void> {
   ...
   * Supports both transaction-scoped (client provided) and standalone mode.
   */
  async createDeletionAuditTrail(
    ...
    const executeAudit = async (c: PoolClient): Promise<void> => {
      ...
      const firstUserResult = userResults.rows[0];
