...
const GET_USER_TENANT_QUERY = 'SELECT tenant_id FROM users WHERE id = $1 AND is_active = 1';
...
export class AdminPermissionsController {
  ...
  async setPermissions(
    ...
    // Get the admin's tenant ID
    const [adminRows] = await execute<RowDataPacket[]>(GET_ADMIN_TENANT_QUERY, [dto.adminId]);
  ...
  async setFullAccess(
    ...
    return { message: dto.hasFullAccess ? 'Full access granted' : 'Full access revoked' };
  }
}
