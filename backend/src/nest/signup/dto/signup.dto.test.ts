...
describe('SignupSchema', () => {
  ...
  describe('subdomain validation', () => {
    it('should reject subdomain under 3 chars', () => {
      expect(SignupSchema.safeParse({ ...valid, subdomain: 'ab' }).success).toBe(false);
    });

    it('should reject subdomain over 50 chars', () => {
      expect(SignupSchema.safeParse({ ...valid, subdomain: 'a'.repeat(51) }).success).toBe(false);
    });

    it('should reject subdomain with uppercase', () => {
      expect(SignupSchema.safeParse({ ...valid, subdomain: 'Acme' }).success).toBe(false);
    });
    ...
    it('should accept subdomain with hyphens in middle', () => {
      expect(SignupSchema.safeParse({ ...valid, subdomain: 'acme-corp' }).success).toBe(true);
    });
  });

  describe('company name validation', () => {
    it('should reject company name under 2 chars', () => {
      expect(SignupSchema.safeParse({ ...valid, companyName: 'A' }).success).toBe(false);
    });

    it('should accept German characters', () => {
      expect(SignupSchema.safeParse({ ...valid, companyName: 'Müller & Söhne GmbH' }).success).toBe(
        true,
      );
    });
  });
  ...
  describe('phone validation', () => {
    it('should accept German phone number', () => {
      expect(SignupSchema.safeParse({ ...valid, phone: '+49 (0)123 456-789' }).success).toBe(true);
    });
  ...
  describe('admin password validation', () => {
    it('should reject password without uppercase', () => {
      expect(SignupSchema.safeParse({ ...valid, adminPassword: 'nouppercase1' }).success).toBe(
        false,
      );
    });

    it('should reject password without lowercase', () => {
      expect(SignupSchema.safeParse({ ...valid, adminPassword: 'NOLOWERCASE1' }).success).toBe(
        false,
      );
    });

    it('should reject password without number', () => {
      expect(SignupSchema.safeParse({ ...valid, adminPassword: 'NoNumberHere!' }).success).toBe(
        false,
      );
    });

    it('should reject password under 8 chars', () => {
      expect(SignupSchema.safeParse({ ...valid, adminPassword: 'Ab1!' }).success).toBe(false);
    });
  });
  ...
  describe('admin name validation', () => {
    ...
    it('should reject empty name', () => {
      expect(SignupSchema.safeParse({ ...valid, adminFirstName: '' }).success).toBe(false);
    });
  });
  ...
  describe('street validation', () => {
    it('should accept German street with umlauts', () => {
      expect(SignupSchema.safeParse({ ...valid, street: 'Königstraße' }).success).toBe(true);
    });

    it('should accept street with numbers', () => {
      expect(SignupSchema.safeParse({ ...valid, street: 'Straße des 17. Juni' }).success).toBe(
        true,
      );
    });

    it('should accept street with slash', () => {
      expect(SignupSchema.safeParse({ ...valid, street: 'Haupt-/Nebenstraße' }).success).toBe(true);
    });

    it('should reject empty street', () => {
      expect(SignupSchema.safeParse({ ...valid, street: '' }).success).toBe(false);
    });
  ...
  describe('houseNumber validation', () => {
    it('should accept numeric house number', () => {
      expect(SignupSchema.safeParse({ ...valid, houseNumber: '42' }).success).toBe(true);
    });

    it('should accept alphanumeric (e.g. "12a")', () => {
      expect(SignupSchema.safeParse({ ...valid, houseNumber: '12a' }).success).toBe(true);
    });

    it('should accept slash notation (e.g. "5/3")', () => {
      expect(SignupSchema.safeParse({ ...valid, houseNumber: '5/3' }).success).toBe(true);
    });

    it('should accept range (e.g. "10-12")', () => {
      expect(SignupSchema.safeParse({ ...valid, houseNumber: '10-12' }).success).toBe(true);
    });

    it('should reject empty', () => {
      expect(SignupSchema.safeParse({ ...valid, houseNumber: '' }).success).toBe(false);
    });

    it('should reject special characters', () => {
      expect(SignupSchema.safeParse({ ...valid, houseNumber: '42!' }).success).toBe(false);
    });
  });
  ...
  describe('postalCode validation', () => {
    it('should accept German PLZ (5 digits)', () => {
      expect(SignupSchema.safeParse({ ...valid, postalCode: '10115' }).success).toBe(true);
    });

    it('should accept Austrian PLZ (4 digits)', () => {
      expect(SignupSchema.safeParse({ ...valid, postalCode: '1010' }).success).toBe(true);
    });

    it('should accept UK postcode with space', () => {
      expect(SignupSchema.safeParse({ ...valid, postalCode: 'SW1A 1AA' }).success).toBe(true);
    });

    it('should accept US ZIP+4 with hyphen', () => {
      expect(SignupSchema.safeParse({ ...valid, postalCode: '10001-1234' }).success).toBe(true);
    });

    it('should accept Canadian postal code', () => {
      expect(SignupSchema.safeParse({ ...valid, postalCode: 'K1A 0B1' }).success).toBe(true);
    });

    it('should reject too short (< 3 chars)', () => {
      expect(SignupSchema.safeParse({ ...valid, postalCode: '12' }).success).toBe(false);
    });

    it('should reject special characters', () => {
      expect(SignupSchema.safeParse({ ...valid, postalCode: '101!5' }).success).toBe(false);
    });
  ...
  describe('city validation', () => {
    it('should accept standard city', () => {
      expect(SignupSchema.safeParse({ ...valid, city: 'Berlin' }).success).toBe(true);
    });

    it('should accept city with umlauts', () => {
      expect(SignupSchema.safeParse({ ...valid, city: 'München' }).success).toBe(true);
    });

    it('should accept compound city name', () => {
      expect(SignupSchema.safeParse({ ...valid, city: 'Frankfurt am Main' }).success).toBe(true);
    });

    it('should accept city with apostrophe', () => {
      expect(SignupSchema.safeParse({ ...valid, city: "L'Aquila" }).success).toBe(true);
    });
  ...
  describe('countryCode validation', () => {
    for (const code of ['DE', 'AT', 'CH', 'US', 'GB', 'FR', 'NL']) {
      it(`should accept ${code}`, () => {
        expect(SignupSchema.safeParse({ ...valid, countryCode: code }).success).toBe(true);
      });
    }

    it('should reject lowercase', () => {
      expect(SignupSchema.safeParse({ ...valid, countryCode: 'de' }).success).toBe(false);
    });

    it('should reject single character', () => {
      expect(SignupSchema.safeParse({ ...valid, countryCode: 'D' }).success).toBe(false);
    });

    it('should reject three characters', () => {
      expect(SignupSchema.safeParse({ ...valid, countryCode: 'DEU' }).success).toBe(false);
    });

    it('should reject numbers', () => {
      expect(SignupSchema.safeParse({ ...valid, countryCode: '49' }).success).toBe(false);
    });

    it('should reject empty', () => {
      expect(SignupSchema.safeParse({ ...valid, countryCode: '' }).success).toBe(false);
    });
  });
  ...
  describe('optional address fields (completed in /settings/company)', () => {
    const addressFields = ['street', 'houseNumber', 'postalCode', 'city', 'countryCode'] as const;
...
describe('CheckSubdomainParamSchema', () => {
  it('should accept valid subdomain', () => {
    expect(CheckSubdomainParamSchema.safeParse({ subdomain: 'acme' }).success).toBe(true);
  });
  ...
  it('should reject too short subdomain', () => {
    expect(CheckSubdomainParamSchema.safeParse({ subdomain: 'ab' }).success).toBe(false);
  });
