...
function createMockMachineRow(overrides?: Partial<DbMachineRow>): DbMachineRow {
  return {
    id: 1,
    tenant_id: 10,
...
describe('buildMachineStringFields', () => {
  it('should return empty object when all fields are null', () => {
    const row = createMockMachineRow();
    expect(buildMachineStringFields(row)).toEqual({});
  });

  it('should include model when not null', () => {
    const row = createMockMachineRow({ model: 'XR-500' });
    expect(buildMachineStringFields(row)).toEqual(
  ...
  it('should include all non-null string fields', () => {
    ...
    const result = buildMachineStringFields(row);
...
describe('buildMachineDateFields', () => {
  it('should return empty object when all date fields are null', () => {
    const row = createMockMachineRow();
    expect(buildMachineDateFields(row)).toEqual({});
  });

  it('should convert purchase_date to ISO string', () => {
    const date = new Date('2024-03-15T00:00:00Z');
    const row = createMockMachineRow({ purchase_date: date });
    const result = buildMachineDateFields(row);
  ...
  it('should convert all non-null date fields', () => {
    ...
    const row = createMockMachineRow(dates);
    const result = buildMachineDateFields(row);
...
describe('buildMachineReferenceFields', () => {
  it('should return empty object when all reference fields are null/undefined', () => {
    const row = createMockMachineRow();
    expect(buildMachineReferenceFields(row)).toEqual({});
  });

  it('should include department_id when not null', () => {
    const row = createMockMachineRow({ department_id: 5 });
    const result = buildMachineReferenceFields(row);
    expect(result.departmentId).toBe(5);
  });

  it('should include department_name when defined', () => {
    const row = createMockMachineRow({ department_name: 'Engineering' });
    const result = buildMachineReferenceFields(row);
    expect(result.departmentName).toBe('Engineering');
  });

  it('should include area fields', () => {
    const row = createMockMachineRow({ area_id: 3, area_name: 'Zone B' });
    const result = buildMachineReferenceFields(row);
  ...
  it('should include operating_hours when not null', () => {
    const row = createMockMachineRow({ operating_hours: 1500 });
    const result = buildMachineReferenceFields(row);
    expect(result.operatingHours).toBe(1500);
  });
});

describe('mapDbMachineToApi', () => {
  it('should map required fields from DB row', () => {
    const row = createMockMachineRow();
    const result = mapDbMachineToApi(row);

    expect(result.id).toBe(1);
    expect(result.tenantId).toBe(10);
    expect(result.name).toBe('CNC Milling 001');
    expect(result.machineType).toBe('production');
  ...
  it('should convert is_active number to boolean', () => {
    const rowActive = createMockMachineRow({ is_active: 1 });
    const rowInactive = createMockMachineRow({ is_active: 0 });

    expect(mapDbMachineToApi(rowActive).isActive).toBe(true);
    expect(mapDbMachineToApi(rowInactive).isActive).toBe(false);
  });
});
...
describe('buildMachineInsertParams', () => {
  it('should return params array with correct length', () => {
    const data: MachineCreateRequest = { name: 'New Machine' };
    const params = buildMachineInsertParams(data, 10, 5, 'uuid-123');
    expect(params).toHaveLength(25);
  });

  it('should place tenantId, name at positions 0 and 1', () => {
    const data: MachineCreateRequest = { name: 'Test' };
    const params = buildMachineInsertParams(data, 10, 5, 'uuid-123');
  ...
  it('should default optional fields to null', () => {
    const data: MachineCreateRequest = { name: 'Minimal' };
    const params = buildMachineInsertParams(data, 10, 5, 'uuid-123');
  ...
  it('should default machineType to production and status to operational', () => {
    const data: MachineCreateRequest = { name: 'Test' };
    const params = buildMachineInsertParams(data, 10, 5, 'uuid-123');
  ...
  it('should convert date strings to Date objects', () => {
    const data: MachineCreateRequest = {
      name: 'Test',
  ...
  it('should place machineUuid as last param', () => {
    const data: MachineCreateRequest = { name: 'Test' };
    const params = buildMachineInsertParams(data, 10, 5, 'my-uuid');
...
describe('buildMachineUpdateFields', () => {
  it('should return empty fields for empty update request', () => {
    const data: MachineUpdateRequest = {};
    const result = buildMachineUpdateFields(data, 5);
  ...
  it('should include date fields when provided', () => {
    const data: MachineUpdateRequest = { purchaseDate: '2024-06-01' };
    const result = buildMachineUpdateFields(data, 5);
  ...
  it('should always append updated_by as the last parameterized field', () => {
    const data: MachineUpdateRequest = { name: 'Test' };
    const result = buildMachineUpdateFields(data, 42);
  ...
  it('should increment paramIndex correctly', () => {
    const data: MachineUpdateRequest = { name: 'A', model: 'B' };
    const result = buildMachineUpdateFields(data, 5);
...
describe('buildMaintenanceInsertParams', () => {
  ...
  it('should place tenantId and machineId at positions 0 and 1', () => {
    const data: MaintenanceRecordRequest = {
      machineId: 7,
