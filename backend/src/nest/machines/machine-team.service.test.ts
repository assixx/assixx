/**
 * Unit tests for MachineTeamService
 *
 * Phase 9: Service tests — mocked DatabaseService.
 * Focus: row mapper (conditional optional fields), validation in setMachineTeams.
 */
...
describe('MachineTeamService', () => {
  let service: MachineTeamService;
  let mockDb: ReturnType<typeof createMockDb>;

  beforeEach(() => {
    mockDb = createMockDb();
    service = new MachineTeamService(mockDb as unknown as DatabaseService);
  });
  ...
  describe('getMachineTeams', () => {
    it('should map DB rows to response with all optional fields', async () => {
      ...
      const result = await service.getMachineTeams(42, 1);
  ...
  describe('setMachineTeams', () => {
    it('should throw BadRequestException when team IDs are invalid', async () => {
      ...
      await expect(service.setMachineTeams(42, [1, 999], 1, 1)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should skip validation and delete when teamIds is empty', async () => {
      // DELETE existing
      mockDb.query.mockResolvedValueOnce([]);
      // getMachineTeams after
      mockDb.query.mockResolvedValueOnce([]);

      const result = await service.setMachineTeams(42, [], 1, 1);

      expect(result).toEqual([]);
      // First call should be DELETE, not SELECT for validation
      const firstCallSql = mockDb.query.mock.calls[0]?.[0] as string;
      expect(firstCallSql).toContain('DELETE FROM machine_teams');
    });
  });
});
