/**
 * Asset Helpers - Pure functions for mapping and query building
 *
...
/** Build WHERE clauses from AssetFilters (pure — no DB access) */
export function buildAssetFilterClauses(
  ...
  if (hasContent(filters.asset_type)) {
    parts.push(`AND m.asset_type = $${idx}`);
  ...
  if (filters.team_id !== undefined && filters.team_id !== 0) {
    parts.push(
      `AND EXISTS (SELECT 1 FROM asset_teams mt2 WHERE mt2.asset_id = m.id AND mt2.team_id = $${idx})`,
    );
...
export function parseTeamsJson(
  teams: AssetTeamInfo[] | string | undefined,
): AssetTeamInfo[] | undefined {
  if (teams === undefined) {
    return undefined;
  }
  ...
  try {
    return JSON.parse(teams) as AssetTeamInfo[];
  } catch {
    return [];
  }
}
...
export function mapDbAssetToApi(row: DbAssetRow): AssetResponse {
  return {
    id: row.id,
    ...
    ...buildAssetStringFields(row),
    ...buildAssetDateFields(row),
    ...buildAssetReferenceFields(row),
  };
}
...
/** Build INSERT parameters for asset creation */
export function buildAssetInsertParams(
...
/** Field mappings for asset update (API field to DB column) */
export const MACHINE_FIELD_MAPPINGS: [keyof AssetUpdateRequest, string][] = [
  ['name', 'name'],
  ['model', 'model'],
