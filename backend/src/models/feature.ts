...
export async function activateFeatureForTenant(
  tenant_id: number,
  featureCode: string,
  options: FeatureActivationOptions = {},
): Promise<boolean> {
  try {
    const feature = await findFeatureByCode(featureCode);
    if (!feature) {
...
export async function deactivateFeatureForTenant(
  tenant_id: number,
  featureCode: string,
): Promise<boolean> {
  try {
...
export async function isFeatureEnabledForTenant(
  ...
  try {
    ...
    return await checkTenantFeatureAccess(tenant_id, featureKey);
  } catch (error: unknown) {
    logger.error(
...
const Feature = {
  findAll: findAllFeatures,
  findByCode: findFeatureByCode,
  checkTenantAccess: checkTenantFeatureAccess,
  activateForTenant: activateFeatureForTenant,
  deactivateForTenant: deactivateFeatureForTenant,
  logUsage: logFeatureUsage,
  getTenantFeatures,
  getUsageStats: getFeatureUsageStats,
  isEnabledForTenant: isFeatureEnabledForTenant,
};
