...
export const validateCreateEmployee: ValidationMiddleware = [
  ...
  body('first_name').notEmpty().trim().escape().withMessage('Vorname darf nicht leer sein'),
  body('last_name').notEmpty().trim().escape().withMessage('Nachname darf nicht leer sein'),
  // Optionale Felder
  body('age')
    .optional({ nullable: true })
    .isInt({ min: 18, max: 100 })
    .withMessage('Alter muss zwischen 18 und 100 liegen'),
  body('employee_id').optional({ nullable: true }).isString().trim().escape(),
  body('department_id')
    .optional({ nullable: true })
    .isNumeric()
...
export const validateLogin: ValidationMiddleware = [
  body('username')
    ...
    .withMessage('Benutzername ist erforderlich und muss 3-50 Zeichen lang sein'),
  body('password')
    .notEmpty()
...
export const validateSignup: ValidationMiddleware = [
  ...
  body('username')
    ...
    .escape()
    .matches(/^[\w-]+$/)
    .withMessage('Benutzername muss 3-30 Zeichen lang sein (nur Buchstaben, Zahlen, _ und -)'),
  body('password')
...
export const validateCreateSurvey: ValidationMiddleware = [
  ...
  body('is_anonymous').optional().isBoolean().withMessage('Anonym muss ein Boolean-Wert sein'),
  body('status')
    .optional()
    .isIn(['draft', 'active', 'completed', 'archived'])
    .withMessage('Status muss draft, active, completed oder archived sein'),
  body('questions').optional().isArray().withMessage('Fragen müssen als Array übermittelt werden'),
  ...
  body('questions.*.question_type')
    .if(body('questions').exists())
    .isIn(['text', 'single_choice', 'multiple_choice', 'number', 'rating', 'date'])
    .withMessage(
...
export const validateSurveyResponse: ValidationMiddleware = [
  body('answers')
    .isArray({ min: 1 })
    .withMessage('Antworten sind erforderlich und müssen als Array übermittelt werden'),
...
export const validateDocumentUpload: ValidationMiddleware = [
  ...
  body('teamId').optional().isInt({ min: 1 }).withMessage('Team-ID muss eine positive Zahl sein'),
  body('departmentId')
    .optional()
    .isInt({ min: 1 })
  ...
  body('category')
    ...
    .withMessage('Kategorie muss salary, contract, certificate oder other sein'),
  body('description')
    .optional()
    .trim()
    .escape()
...
export const validateCreateShift: ValidationMiddleware = [
  body('employee_id')
    .notEmpty()
    .isInt({ min: 1 })
    .withMessage('Mitarbeiter-ID ist erforderlich und muss eine positive Zahl sein'),
  body('start_time')
    .notEmpty()
    .isISO8601()
    .withMessage('Startzeit ist erforderlich und muss ein gültiges Datum/Zeit sein'),
  body('end_time')
    .notEmpty()
    .isISO8601()
    .withMessage('Endzeit ist erforderlich und muss ein gültiges Datum/Zeit sein'),
  body('shift_type')
    .optional()
    .trim()
    .escape()
    .isIn(['morning', 'afternoon', 'night', 'overtime'])
    .withMessage('Schichttyp muss morning, afternoon, night oder overtime sein'),
  body('notes')
    .optional()
    .trim()
    .escape()
...
export const validateCreatePost: ValidationMiddleware = [
  ...
  body('tags').optional().isArray().withMessage('Tags müssen als Array übermittelt werden'),
  body('tags.*')
    .if(body('tags').exists())
    .trim()
    .escape()
...
export const validateIdParam: ValidationMiddleware = [
  body('id').optional().isInt({ min: 1 }).withMessage('ID muss eine positive Zahl sein'),
  handleValidationErrors,
];
...
export const validateFileUpload = (allowedTypes: string[], maxSize: number) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (fileExtension == null || fileExtension === '' || !allowedTypes.includes(fileExtension)) {
      res.status(400).json({
