...
describe('Authentication Middleware - Integration Test', () => {
  ...
  describe('Token Validation', () => {
    it('should authenticate valid Bearer token', async () => {
      ...
      await authenticateToken(mockRequest as PublicRequest, mockResponse as Response, mockNext);
  ...
  describe('User Status Validation', () => {
    it('should reject inactive user', async () => {
      // Make user inactive
      await pool.execute('UPDATE users SET is_active = 0 WHERE id = ?', [testUserId]);
      ...
      await pool.execute('UPDATE users SET is_active = 1 WHERE id = ?', [testUserId]);
    });
