...
/**
 * Extracts JWT token from request headers, cookies, or query parameters
 * Query parameter support is for iframe document previews (same-origin only)
...
function extractToken(req: PublicRequest): string | null {
  ...
  // Check query parameter as last resort (for iframe previews)
  // Safe: Only used for same-origin document preview iframes (X-Frame-Options: SAMEORIGIN)
  const queryToken = req.query.token as string | undefined;

  return bearerToken ?? cookieToken ?? queryToken ?? null;
}
