import { __require } from './chunk-FQY4JAKU.js';
...
function zeroFill(number, targetLength, forceSign) {
  var absNumber = '' + Math.abs(number),
    ...
    sign2 = number >= 0;
  return (
    (sign2 ? (forceSign ? '+' : '') : '-') + Math.pow(10, Math.max(0, zerosToFill)).toString().substr(1) + absNumber
  );
}
...
function localeMonths(m, format2) {
  ...
  return isArray(this._months)
    ...
    : this._months[(this._months.isFormat || MONTHS_IN_FORMAT).test(format2) ? 'format' : 'standalone'][m.month()];
}
