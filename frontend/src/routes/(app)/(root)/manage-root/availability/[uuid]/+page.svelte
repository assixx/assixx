<script lang="ts">
  /**
   * Availability History - Page Component (Root)
   * @module manage-root/availability/[uuid]/+page
   *
   * Displays availability history for a specific root user.
   */
  ...
  function navigateWithFilter(year: string, month: string): void {
    ...
    void goto(
      resolve(`/manage-root/availability/${uuid}${queryString !== '' ? `?${queryString}` : ''}`),
    );
  }
...
<div class="container">
  ...
  <div class="mb-4">
    <button
      ...
      onclick={goBack}
    >
      <i class="fas fa-arrow-left mr-2"></i>Zurück zur Root-Verwaltung
    </button>
  </div>
