<script lang="ts">
  /**
   * Machine Availability History - Page Component
   * @module manage-machines/availability/[uuid]/+page
   *
   * Displays availability history for a specific machine.
  ...
  import DeleteConfirmationModal from '$lib/machine-availability/DeleteConfirmationModal.svelte';
  import EditMachineAvailabilityModal from '$lib/machine-availability/EditMachineAvailabilityModal.svelte';
  import {
    formatDate,
    formatDateTime,
    getStatusClass,
    ...
    getStatusText,
    truncateText,
  } from '$lib/machine-availability/helpers';
  ...
  const entries = $derived(data.entries as MachineAvailabilityEntry[]);
  ...
  let selectedEntry = $state<MachineAvailabilityEntry | null>(null);
  ...
  function isEditable(entry: MachineAvailabilityEntry): boolean {
  ...
  function goBack(): void {
    void goto(resolvePath('/manage-machines'));
  }
  ...
  function navigateWithFilter(year: string, month: string): void {
    ...
    const uuid = machine?.uuid ?? '';
    void goto(
      resolvePath(
        `/manage-machines/availability/${uuid}${queryString !== '' ? `?${queryString}` : ''}`,
      ),
    );
  }
  ...
  function openEditModal(entry: MachineAvailabilityEntry): void {
    selectedEntry = entry;
    showEditModal = true;
  }
  ...
  function openDeleteModal(entry: MachineAvailabilityEntry): void {
    selectedEntry = entry;
...
<svelte:head>
  <title>Maschinenverfügbarkeit - {machine?.name ?? ''} - Assixx</title>
</svelte:head>
...
<div class="container">
  ...
  <div class="mb-4">
    <button
      ...
      onclick={goBack}
    >
      <i class="fas fa-arrow-left mr-2"></i>Zurück zur Maschinenverwaltung
    </button>
  </div>
      ...
      {:else if entries.length === 0}
        <div class="empty-state">
          ...
          <p class="empty-state__description">
            ...
            {:else}
              Es wurden noch keine Verfügbarkeitseinträge für diese Maschine
              erfasst.
            {/if}
          </p>
          <button
          ...
          >
            ...
            Zurück zur Maschinenverwaltung
          </button>
        </div>
      {:else}
