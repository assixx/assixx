<script lang="ts">
  ...
  // Tenant-wide gate from layout SSR: when Root disables
  // `security.allow_user_password_change`, admins cannot self-rotate their
  // password. The backend guard in UserProfileService is authoritative;
  // this `{#if}` wrap is the UX mirror so admins don't see a form that
  // would 403 on submit. See ADR-045 + user-request 2026-04-20.
