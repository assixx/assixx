/**
 * Validation Functions for Shift Planning System
 * Context validation, hierarchy checks, and input validation
 */

import type { SelectedContext, Area, Department, Machine, Team, Employee } from './types';
 ...
 */
export function validateAreaSelection(areas: Area[], selectedAreaId: number | null): ValidationResult | null {
 ...
 */
export function validateDepartmentSelection(selectedDepartmentId: number | null): ValidationResult | null {
...
export function validateDepartmentBelongsToArea(
  ...
  if (department?.areaId !== undefined && department.areaId !== 0 && department.areaId !== selectedAreaId) {
 ...
 */
export function validateSavePrerequisites(isAdmin: boolean, selectedDepartmentId: number | null): ValidationResult {
  if (!isAdmin) {
...
export function isEmployeeAvailableOnDate(employee: Employee, dateString: string): boolean {
  ...
  console.info('[AVAILABILITY DEBUG]', {
    ...
    checkDateLocal: checkDate.toLocaleDateString('de-DE'),
    startISO: start?.toISOString() ?? 'null',
    startLocal: start?.toLocaleDateString('de-DE') ?? 'null',
    endISO: end?.toISOString() ?? 'null',
    endLocal: end?.toLocaleDateString('de-DE') ?? 'null',
    afterStart,
    beforeEnd,
    isUnavailable,
    result: isUnavailable ? 'UNAVAILABLE' : 'AVAILABLE',
  });
 ...
 */
export function getEffectiveAvailabilityForWeek(employee: Employee, weekDates: Date[]): string {
  const rawStatus = employee.availabilityStatus ?? 'available';
...
export function isEmployeeUnavailableInWeek(employee: Employee, weekDates: Date[]): boolean {
  return getEffectiveAvailabilityForWeek(employee, weekDates) !== 'available';
}
