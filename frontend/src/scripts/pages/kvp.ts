...
class KvpPage {
  ...
  private async loadStatistics(): Promise<void> {
    try {
      ...
      document.getElementById('openSuggestions')!.textContent = 
        ((data.company.byStatus.new || 0) + (data.company.byStatus.in_review || 0)).toString();
      document.getElementById('implementedSuggestions')!.textContent = 
        (data.company.byStatus.implemented || 0).toString();
  ...
  private openCreateModal(): void {
    ...
    if (categoryDropdown && this.categories.length > 0) {
      categoryDropdown.innerHTML = this.categories.map(category => `
