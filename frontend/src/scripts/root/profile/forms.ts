...
/**
 * Root Profile Management - Forms Layer
...
// displayInitials removed - page reload handles avatar update (2025-11-20)
 ...
 */
export function setupProfileForm(): void {
  const profileForm = $$id('profile-form') as HTMLFormElement | null;
  if (profileForm === null) {
    return;
  }
  ...
  profileForm.addEventListener('submit', (e) => {
    ...
    void (async () => {
      ...
      const data = Object.fromEntries(formData) as ProfileUpdateData;

      try {
        await handleUpdateProfile(data);
        showSuccessAlert('Profil erfolgreich aktualisiert');
        ...
        console.error('[RootProfile] Error updating profile:', error);
        const errorObj = error as { message?: string };
        showErrorAlert(errorObj.message ?? 'Fehler beim Aktualisieren');
      }
    })();
  });
}
...
function handlePasswordChangeError(error: unknown): void {
  console.error('[RootProfile] Password change error:', error);
  ...
  pictureInput.addEventListener('change', (e) => {
    ...
    void (async () => {
      try {
        ...
        console.error('[RootProfile] Error uploading profile picture:', error);
  ...
  removePictureBtn.addEventListener('click', () => {
    void (async () => {
      ...
      try {
        ...
        console.error('[RootProfile] Error removing profile picture:', error);
