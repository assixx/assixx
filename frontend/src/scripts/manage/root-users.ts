...
function renderEmptyState(): string {
  return `<div class="empty-state"><div class="empty-state-icon"><i class="fas fa-user-shield"></i></div><div class="empty-state-text">Keine weiteren Root-Benutzer vorhanden</div><div class="empty-state-subtext">Sie sind der einzige Root-Benutzer. Klicken Sie auf den + Button, um weitere Root-Benutzer hinzuzufügen.</div></div><div style="margin-top:16px;padding:10px;background:rgba(255,193,7,0.1);border:1px solid rgba(255,193,7,0.3);border-radius:8px"><p style="margin:0;color:#ffc107;font-size:14px"><i class="fas fa-exclamation-triangle" style="margin-right:8px"></i>Ihr eigenes Profil wird hier nicht angezeigt. Bearbeiten Sie es über <a href="/root-profile" style="color:#ffc107;text-decoration:underline">Mein Profil</a>.</p></div>`;
}

function renderUserRow(user: RootUser): string {
  const status = user.isActive === true ? 'active' : 'inactive';
  const statusText = user.isActive === true ? 'Aktiv' : 'Inaktiv';
  const created = new Date(user.createdAt).toLocaleDateString('de-DE');
  const lastLogin = user.lastLogin !== undefined ? new Date(user.lastLogin).toLocaleDateString('de-DE') : '-';
  return `<tr><td>${user.id}</td><td>${user.firstName} ${user.lastName}</td><td>${user.email}</td><td>${user.position ?? '-'}</td><td><span class="status-badge ${status}">${statusText}</span></td><td>${created}</td><td>${lastLogin}</td><td><button class="action-btn edit" data-action="edit-root-user" data-user-id="${user.id}">Bearbeiten</button><button class="action-btn permissions" data-action="show-root-permissions" data-user-id="${user.id}" style="border-color:rgba(76,175,80,0.3);background:rgba(76,175,80,0.1)">Berechtigungen</button><button class="action-btn delete" data-action="delete-root-user" data-user-id="${user.id}">Löschen</button></td></tr>`;
}

function renderProfileInfo(): string {
  return `<div style="margin-top:16px;padding:10px;background:rgba(255,255,255,0.02);border-radius:8px;border:1px solid rgba(255,255,255,0.1)"><p style="margin:0;color:var(--text-secondary);font-size:14px"><i class="fas fa-info-circle" style="margin-right:8px;color:var(--primary-color)"></i>Ihr eigenes Profil wird hier nicht angezeigt. Bearbeiten Sie es über <a href="/root-profile" style="color:var(--primary-color);text-decoration:underline">Mein Profil</a>.</p></div>`;
}
...
function displayRootUsers(users: RootUser[]): void {
  ...
  setHTML(
    container,
    `<table class="root-table"><thead><tr><th>ID</th><th>Name</th><th>E-Mail</th><th>Position</th><th>Status</th><th>Erstellt am</th><th>Letzter Login</th><th>Aktionen</th></tr></thead><tbody>${rows}</tbody></table>${renderProfileInfo()}`,
  );
}
...
function buildUserData(values: FormValues, isEdit: boolean): Record<string, unknown> {
  ...
  if (values.employeeNumber !== '') userData['employeeNumber'] = values.employeeNumber;
  if (values.departmentId !== '') userData['departmentId'] = Number.parseInt(values.departmentId, 10);
  if (!isEdit) {
...
async function handleFormSubmit(event: Event): Promise<void> {
  ...
  try {
    ...
    await apiClient.request(
      endpoint,
      { method: isEdit ? 'PUT' : 'POST', body: JSON.stringify(userData) },
      { version: 'v2' },
    );
...
function closeRootModal(): void {
  ...
  currentEditId = null;
  ($('#rootForm') as HTMLFormElement).reset();
  clearErrorMessages();
}
...
function showAddRootModal(): void {
  ...
  if (modalTitle !== null) modalTitle.textContent = 'Root User hinzufügen';
  ($('#rootForm') as HTMLFormElement).reset();
  const positionDropdown = $('#positionDropdownDisplay') as HTMLElement | null;
  const positionSpan = positionDropdown?.querySelector('span');
  if (positionSpan !== null && positionSpan !== undefined) positionSpan.textContent = 'Position auswählen...';
  const passwordGroup = $('#passwordGroup') as HTMLElement | null;
  if (passwordGroup !== null) passwordGroup.style.display = 'block';
  const passwordConfirmGroup = $('#passwordConfirmGroup') as HTMLElement | null;
  if (passwordConfirmGroup !== null) passwordConfirmGroup.style.display = 'block';
  const activeStatusGroup = $('#activeStatusGroup') as HTMLElement | null;
  if (activeStatusGroup !== null) activeStatusGroup.style.display = 'none';
  clearErrorMessages();
...
async function editRootUserHandler(userId: number): Promise<void> {
  ...
  if (employeeNumberInput !== null) employeeNumberInput.value = user.employeeNumber ?? '';
  const departmentSelect = $('#rootDepartmentId') as HTMLSelectElement | null;
  ...
  if (user.position !== undefined && user.position !== '') {
    ...
    if (positionSpan !== null) positionSpan.textContent = user.position;
  }
  $('#passwordGroup').style.display = 'none';
  $('#passwordConfirmGroup').style.display = 'none';
  $('#activeStatusGroup').style.display = 'block';
  ($('#rootIsActive') as HTMLInputElement).checked = Boolean(user.isActive);
  $('#rootModal').classList.add('active');
}
