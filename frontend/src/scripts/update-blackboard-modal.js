...
function convertSelectsToDropdowns() {
  ...
  if (orgLevelSelect && !document.getElementById('orgLevelDropdown')) {
    const customDropdown = createCustomDropdown('orgLevel', [
  ...
  if (prioritySelect && !document.getElementById('priorityDropdown')) {
    const customDropdown = createCustomDropdown('priority', [
...
async function loadOrgOptions(type) {
  ...
  try {
    ...
    dropdownOptions.innerHTML = items.map(item => 
      `<div class="dropdown-option" onclick="selectOrgId('${item.id}', '${item.name}')">${item.name}</div>`
    ).join('');
  } catch (error) {
