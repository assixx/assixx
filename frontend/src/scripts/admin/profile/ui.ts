/**
 * Admin Profile Management - UI Layer
...
/**
 * Fill profile form fields with user data (readonly for admins)
 */
export function fillFormFields(userData: AdminProfile): void {
  ...
  const companyInput = $$id('company') as HTMLInputElement | null;
  ...
  if (companyInput !== null) {
    companyInput.value = userData.companyName ?? '-';
  }
}
...
export function displayInitials(firstName: string, lastName: string): void {
  ...
  try {
    ...
    console.warn('[AdminProfile] Failed to clear profile picture cache:', e);
  }
  ...
  console.info('[AdminProfile] Setting initials:', { firstName, lastName, initials });
...
export function displayProfilePicture(url: string): void {
  ...
  try {
    ...
    console.warn('[AdminProfile] Failed to cache profile picture URL:', e);
  }
