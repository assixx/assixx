...
function populateProfileForm(profile: UserProfile): void {
  ...
  (document.getElementById('position') as HTMLInputElement).value = 
    profile.position ? (positionMap[profile.position] || profile.position) : '-';
...
async function uploadProfilePicture(file: File): Promise<void> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const pictureUrl = result.profilePictureUrl || result.profile_picture_url || result.url || result.data?.profile_picture_url;
      updateProfilePicture(pictureUrl);
