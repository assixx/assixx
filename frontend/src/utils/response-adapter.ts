...
// Snake_case to camelCase conversion map
...
// CamelCase to snake_case conversion map (reverse of above)
const CAMEL_TO_SNAKE_MAP: Record<string, string> = Object.entries(SNAKE_TO_CAMEL_MAP).reduce<Record<string, string>>(
  (acc, [snake, camel]) => {
    // eslint-disable-next-line security/detect-object-injection -- camel is from our predefined SNAKE_TO_CAMEL_MAP, not user input
    acc[camel] = snake;
    return acc;
  },
  {},
...
const ResponseAdapterInternal = {
  ...
  snakeToCamel(str: string): string {
    ...
    return SNAKE_TO_CAMEL_MAP[str] ?? str.replace(/_([a-z])/g, (_, letter: string) => letter.toUpperCase());
  },
  ...
  camelToSnake(str: string): string {
    // eslint-disable-next-line security/detect-object-injection -- str is a controlled field name from internal data structures, not user input
    return CAMEL_TO_SNAKE_MAP[str] ?? str.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`);
  },

  /**
   * Convert date fields
   */
  convertDateFields(data: Record<string, unknown>): void {
    const dateFields = [
      'createdAt',
      'updatedAt',
      'deletedAt',
   ...
   */
  convertNumericFields(data: Record<string, unknown>): void {
    const numericFields = [
      'id',
      'userId',
      'tenantId',
   ...
   */
  convertBooleanFields(data: Record<string, unknown>): void {
    const booleanFields = [
      'isActive',
      'isDeleted',
      'isAdmin',
  ...
  applyV2FieldConversions(data: Record<string, unknown>): Record<string, unknown> {
    ...
    this.convertNumericFields(data);
