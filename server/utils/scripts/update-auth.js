...
async function updateAuthImplementation() {
  ...
  try {
    ...
    const affectedFiles = grepOutput
      .split('\n')
      ...
      .filter(
        (filePath) =>
          filePath !== 'auth-unified.js' && filePath !== 'update-auth.js'
      );
    ...
    for (const filePath of uniqueFiles) {
      try {
        ...
        error(
          `Fehler beim Aktualisieren von ${filePath}: ${fileError.message}`
        );
      }
    }
    ...
    try {
      ...
      error(
        `Fehler beim Sichern von middleware/auth.js: ${backupError.message}`
      );
    }
    ...
    try {
      ...
      error(
        `Fehler beim Aktivieren der neuen Auth-Implementierung: ${activateError.message}`
      );
    }

    log(
      'Auth-Implementierung erfolgreich aktualisiert! Starte den Server neu, um die Änderungen zu übernehmen.'
    );
  } catch (generalError) {
