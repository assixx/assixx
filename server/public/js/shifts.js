...
function setupEventListeners() {
  ...
  const createShiftTemplateBtn = document.getElementById(
    'createShiftTemplateBtn'
  );
  if (createShiftTemplateBtn) {
    createShiftTemplateBtn.addEventListener('click', createShiftTemplate);
  }

  const createExchangeRequestBtn = document.getElementById(
    'createExchangeRequestBtn'
  );
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById('totalShifts').textContent =
        data.stats.totalUpcomingShifts || 0;
      document.getElementById('assignedEmployees').textContent =
        data.upcomingShifts.length || 0;
      document.getElementById('pendingExchanges').textContent =
        data.stats.pendingExchanges || 0;
...
function displayUpcomingShifts(shifts) {
  ...
  if (shifts.length === 0) {
    container.innerHTML =
      '<div class="no-data">Keine geplanten Schichten gefunden.</div>';
    return;
  }
...
function displayWeeklyAvailability(availability) {
  ...
  const availabilityHtml = weekDays
    .map((date) => {
      const dateStr = date.toISOString().split('T')[0];
      const dayAvailability = availability.find((a) => a.date === dateStr);
      const isAvailable = dayAvailability
        ? dayAvailability.availability_type === 'available'
        : true;
...
function displayExchangeRequests(requests) {
  ...
  if (requests.length === 0) {
    container.innerHTML =
      '<div class="no-data">Keine offenen Tauschanfragen.</div>';
    return;
  }
...
function displayShiftTemplates() {
  ...
  if (shiftTemplates.length === 0) {
    container.innerHTML =
      '<div class="no-data">Keine Schichtvorlagen gefunden. Erstellen Sie eine neue Vorlage.</div>';
    return;
  }

  const templatesHtml = shiftTemplates
    .map(
      (template) => `
...
function populateDepartmentSelects() {
  const selects = document.querySelectorAll(
    '#planDepartmentFilter, #planDepartment'
  );
...
async function loadShiftPlans() {
  try {
    ...
    container.innerHTML =
      '<div class="loading">Schichtpläne werden geladen...</div>';
    ...
    if (response.ok) {
      ...
      container.innerHTML =
        '<div class="error">Fehler beim Laden der Schichtpläne</div>';
    }
  } catch (error) {
    console.error('Error loading shift plans:', error);
    document.getElementById('shiftPlansList').innerHTML =
      '<div class="error">Fehler beim Laden der Schichtpläne</div>';
  }
}
...
function displayShiftPlans() {
  ...
  if (shiftPlans.length === 0) {
    container.innerHTML =
      '<div class="no-data">Keine Schichtpläne gefunden. Erstellen Sie einen neuen Plan.</div>';
    return;
  }

  const plansHtml = shiftPlans
    .map(
      (plan) => `
...
function createShiftPlan() {
  document.getElementById('shiftPlanModalTitle').textContent =
    'Neuer Schichtplan';
...
function createShiftTemplate() {
  document.getElementById('shiftTemplateModalTitle').textContent =
    'Neue Schichtvorlage';
 ...
 */
document
  .getElementById('shiftTemplateForm')
  .addEventListener('submit', async function (e) {
    e.preventDefault();
    ...
    try {
      ...
      if (
        !templateData.duration_hours &&
        templateData.start_time &&
        templateData.end_time
      ) {
        templateData.duration_hours = calculateDuration(
          templateData.start_time,
          templateData.end_time
        );
      }
...
function loadAssignments() {
  document.getElementById('assignmentsList').innerHTML =
    '<div class="no-data">Zuweisungen werden in der nächsten Version implementiert.</div>';
}

function loadExchangeRequests() {
  document.getElementById('exchangeRequestsList').innerHTML =
    '<div class="no-data">Tauschbörse wird in der nächsten Version implementiert.</div>';
}
