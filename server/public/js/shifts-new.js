...
class ShiftPlanningSystem {
    ...
    async checkUserRole() {
        try {
            ...
            if (user) {
                ...
                if (user.department) {
                    document.getElementById('currentDepartment').textContent = user.department;
                }
                if (user.position) {
                    document.getElementById('currentTeamLeader').textContent = user.name || user.username;
                }
            }
        } catch (error) {
        ...
        document.addEventListener('dragstart', (e) => {
            if (e.target.closest('.employee-item')) {
                ...
                e.dataTransfer.setData('text/plain', JSON.stringify({
                    ...
                    restrictions: employeeItem.getAttribute('data-employee-restrictions')
                }));
                
                console.log('Drag started for employee:', employeeItem.getAttribute('data-employee-name'));
            }
        });
    ...
    validateEmployeeDrop(employeeData, shiftType, day) {
        ...
        if (employeeData.status === 'unavailable') {
            return { valid: false, message: `${employeeData.name} ist nicht verfügbar` };
        }
        
        // Check shift restrictions
        if (employeeData.status === 'limited') {
            if (employeeData.restrictions === 'early' && shiftType !== 'early') {
                return { valid: false, message: `${employeeData.name} kann nur Frühschicht arbeiten` };
            }
        }
        ...
        if (this.weeklyShifts[day]) {
            for (const existingShiftType of Object.keys(this.weeklyShifts[day])) {
                if (existingShiftType !== shiftType && this.weeklyShifts[day][existingShiftType].employeeId == employeeData.id) {
                    const shiftNames = {
                        'early': 'Frühschicht',
                        'late': 'Spätschicht',
                        'night': 'Nachtschicht'
    ...
    showDropError(employeeData, shiftType) {
        ...
        } else if (employeeData.status === 'limited' && employeeData.restrictions === 'early' && shiftType !== 'early') {
    ...
    setupContextEvents() {
        ...
        // Machine selection
        document.getElementById('machineSelect').addEventListener('change', async (e) => {
            this.selectedContext.machineId = e.target.value;
            await this.onContextChange();
        });

        // Team leader selection
        document.getElementById('teamLeaderSelect').addEventListener('change', async (e) => {
    ...
    setupNotesEvents() {
        ...
        if (!this.isAdmin) {
            ...
            notesTextarea.placeholder = 'Notizen können nur von Admins bearbeitet werden';
            return;
        }
    ...
    async loadTeamLeaders() {
        try {
            ...
            if (response.ok) {
                ...
                this.teamLeaders = (data.users || []).filter(user => 
    ...
    populateMachineSelect() {
        ...
        if (this.selectedContext.departmentId) {
            filteredMachines = this.machines.filter(machine => 
                machine.department_id == this.selectedContext.departmentId
            );
        }
    ...
    async loadEmployees() {
        try {
            ...
            if (response.ok) {
                ...
                this.employees = (data.users || []).filter(user => 
    ...
    renderEmployeeList() {
        ...
        this.employees.forEach(employee => {
            ...
            if (employee.restrictions && employee.restrictions.includes('Frühschicht')) {
    ...
    assignEmployeeToShift(shiftCell) {
        ...
        // Get employee data
        const employee = this.employees.find(emp => emp.id == this.selectedEmployee.id);
        if (!employee) return;
        ...
        console.log(`Assigned ${this.selectedEmployee.name} to ${day} ${shift} shift`);
    ...
    updateWeekDisplay() {
        ...
        for (let i = 1; i < dayHeaders.length; i++) { // Skip first header (Schicht)
    ...
    async loadCurrentWeekData() {
        ...
        try {
            ...
            const response = await fetch(`/api/shifts/weekly?start_date=${startDate}&end_date=${endDate}`, {
                headers: {
                    'Authorization': `Bearer ${getAuthToken()}`,
                    'Content-Type': 'application/json'
                }
    ...
    async loadEmployeeShifts() {
        try {
            ...
            const response = await fetch(`/api/shifts/my-shifts?start_date=${startDate}&end_date=${endDate}`, {
                headers: {
                    'Authorization': `Bearer ${getAuthToken()}`,
                    'Content-Type': 'application/json'
                }
    ...
    displayWeeklyShifts(shifts) {
        ...
        shifts.forEach(shift => {
            ...
            if (dayName && shiftType) {
                const cell = document.querySelector(`[data-day="${dayName}"][data-shift="${shiftType}"]`);
            ...
            for (const shiftType of Object.keys(this.weeklyShifts[day])) {
                ...
                try {
                    ...
                    if (shiftResponse.ok) {
                        ...
                        const assignResponse = await fetch(`/api/shifts/${shiftId}/assign`, {
                            method: 'POST',
                            headers: {
                                'Authorization': `Bearer ${getAuthToken()}`,
                                'Content-Type': 'application/json'
    ...
    async loadWeeklyNotes() {
        try {
            ...
            const response = await fetch(`/api/shifts/weekly-notes?week=${weekNumber}&year=${year}`, {
                headers: {
                    'Authorization': `Bearer ${getAuthToken()}`,
                    'Content-Type': 'application/json'
                }
    ...
    resetSchedule() {
        ...
        if (confirm('Möchten Sie alle Änderungen der aktuellen Woche zurücksetzen?')) {
            this.clearWeekShifts();
            this.selectedEmployee = null;
            document.querySelectorAll('.employee-item.selected').forEach(item => {
                item.classList.remove('selected');
