...
async function createEmployee() {
  try {
    ...
    const placeholders = Object.keys(employeeData).map(() => '?').join(', ');
    ...
    try {
      ...
      console.log(`Mitarbeiter erfolgreich erstellt mit ID: ${result.insertId}`);
    } catch (error) {
      // Wenn der Mitarbeiter bereits existiert, aktualisieren wir ihn stattdessen
      if (error.code === 'ER_DUP_ENTRY') {
        console.log('Mitarbeiter existiert bereits. Aktualisiere stattdessen...');
        ...
        console.log(`Mitarbeiter erfolgreich aktualisiert. Betroffene Zeilen: ${updateResult.affectedRows}`);
      } else {
        throw error;
      }
    }
