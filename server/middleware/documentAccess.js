 ...
 */
const checkDocumentAccess = (options = { allowAdmin: true, allowDepartmentHeads: false, requireOwnership: true }) => {
  return async (req, res, next) => {
    const userId = req.user.id;
    const userRole = req.user.role;
    const { documentId } = req.params;
    
    logger.info(`Checking document access for user ${userId} (role: ${userRole}) to document ${documentId}`);
    ...
    try {
      ...
      if (isOwner) {
        logger.info(`Document owner ${userId} accessing document ${documentId}`);
        return next();
      }
      
      // Wenn Besitzereigenschaft erforderlich ist und der Benutzer nicht der Besitzer ist, Zugriff verweigern
      if (options.requireOwnership) {
        logger.warn(`Access denied: User ${userId} is not the owner of document ${documentId}`);
      ...
      if (options.allowDepartmentHeads && userRole === 'department_head') {
        ...
        if (user && user.department_id) {
          ...
          if (documentOwner && documentOwner.department_id === user.department_id) {
            logger.info(`Department head ${userId} granted access to document ${documentId} of user ${document.user_id} in same department`);
            return next();
          }
        }
      }
      ...
      logger.warn(`Access denied: User ${userId} does not have permission to access document ${documentId}`);
      return res.status(403).json({ message: 'Zugriff verweigert' });
    } catch (error) {
      logger.error(`Error checking document access: ${error.message}`);
      return res.status(500).json({ message: 'Fehler bei der Berechtigungsprüfung', error: error.message });
    }
  };
};
