...
async function connectToMySQL(user, password) {
  try {
    ...
    if (assixxExists) {
      ...
      console.log('To create it, run these commands in the MySQL command line:');
...
function updateEnvFile(user, password) {
  try {
    ...
    console.log('✅ .env file updated successfully with your MySQL credentials.');
  } catch (error) {
...
function promptUser() {
  rl.question('Enter MySQL username (default: root): ', (user) => {
    ...
    rl.question('Enter MySQL password: ', async (password) => {
      ...
      if (success) {
        rl.question('Do you want to update your .env file with these credentials? (y/n): ', (answer) => {
...
console.log('====================');
console.log('This script will help you test your MySQL connection and update your .env file.');
promptUser();