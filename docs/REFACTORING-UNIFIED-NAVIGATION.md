...
```typescript
import type { Tenant, User } from '../../../types/api.types';
...
import { CSS_CLASSES, DASHBOARD_URLS } from './constants';
...
export function createRootRoleSwitchDropdown(activeRole: string | null): string {
  ...
  const currentRole = (
    activeRole === 'root' ? 'root'
    : activeRole === 'admin' ? 'admin'
    : 'employee') as Role;
  const currentView =
    activeRole === 'root' ? 'Root-Ansicht'
    : activeRole === 'admin' ? 'Admin-Ansicht'
    : 'Mitarbeiter-Ansicht';
...
export function createRoleSwitchBanner(storedUserRole: string | null, activeRole: string | null): string {
  ...
  const currentRoleText =
    activeRole === 'employee' ? 'Mitarbeiter'
    : activeRole === 'admin' ? 'Admin'
    : 'Root';
  const originalRoleText =
    storedUserRole === 'root' ? 'Root'
    : storedUserRole === 'admin' ? 'Admin'
    : 'Mitarbeiter';
...
function updateToggleIcon(collapsed: boolean): void {
  ...
  if (iconPath) {
    const path =
      collapsed ? 'M4,6H20V8H4V6M4,11H15V13H4V11M4,16H20V18H4V16Z' : 'M3,6H21V8H3V6M3,11H21V13H3V11M3,16H21V18H3V16Z';
...
export function getDocumentSubmenu(): NavItem[] {
  return [{ id: 'documents-explorer', icon: getIcon('folder'), label: 'Datei Explorer', url: '/documents-explorer' }];
}
...
_(Wie im ursprünglichen Plan, aber mit imports aus role-switch-ui.ts und sidebar.ts)_
...
// Utils
// Design System
import { getColorClass, getInitials } from '../../../design-system/primitives/avatar/avatar.js';
import { apiClient } from '../../../utils/api-client';
import { $$, $all, setHTML } from '../../../utils/dom-utils';
import { tokenManager } from '../../../utils/token-manager';
import { loadUserInfo, logout } from '../../auth/index';
// Auth
import { switchRoleForAdmin, switchRoleForRoot } from '../../auth/role-switch';
// Services
