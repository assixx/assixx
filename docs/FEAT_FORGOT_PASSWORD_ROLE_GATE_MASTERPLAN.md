...
> **Plan type:** FEATURE
> **Created:** 2026-04-16
> **Version:** 0.4.3 (Staff-Engineer Scope-Re-Cut — three features dropped after user-directed KISS-pass: (1) Per-Email-Throttle deferred to v2 (YAGNI, IP-throttle stays), (2) Clean-Break Response reverted to additive `{ message, blocked?, reason? }`, (3) TRUNCATE deploy-step removed (redemption-gate burns pre-existing admin tokens naturally). Plan shrinks from 4+0.3 → 3 Sessions. The v0.4.2 `authEmailTracker` bug that triggered this review is moot — the feature is out.)
...
| ------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---- |
| 1       | 0 + 2     | Current-state analysis (anchors, DTO, frontend, CLS extension) + Backend (both gates, email template, audit wiring, CLS extension). v0.4.3: no new throttler decorator — `@AuthThrottle()` stays. |        |      |
| 2       | 3 + 4 + 5 | Unit tests (≥ 10) + API integration tests (≥ 8) + Frontend (merge-not-replace, toast pattern, additive response).                                                                                 |        |      |
| 3       | 6         | ADR-047 + smoke (dev + production). v0.4.3: no TRUNCATE deploy-step.                                                                                                                              |        |      |
