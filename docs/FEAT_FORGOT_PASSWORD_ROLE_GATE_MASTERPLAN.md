...
> **Plan type:** FEATURE
> **Created:** 2026-04-16
> **Version:** 0.5.2 (Session 1c complete — Root-Initiated Reset backend landed: Phase 1 migration `add-password-reset-initiated-by` applied (column + FK ON DELETE SET NULL verified, pgmigrations row #140, customer fresh-install synced), §2.7 endpoint `POST /api/v2/users/:id/send-password-reset-link` with strict `@Roles('root')` + `sendAdminInitiatedResetLink()` service method with 4 reject-branches + per-pair DB rate-limit, §2.8 `resetPassword()` origin-check branch in `enforceRedemptionOriginGate()` helper (admin-initiated tokens bypass §2.6 role-gate, initiator-lifecycle check burns stale tokens), §2.9 `sendPasswordResetAdminInitiated()` mailer + template. ESLint + TS + 6767/6767 unit tests + backend-boot-health all green.)
> **Status:** PHASE 2 Session 1c DONE (admin-initiated backend) — ready for Session 2 (tests ~17 unit + ~14 API, frontend §5.1+§5.2 merge + §5.3+§5.4 manage-admin/employee buttons) + Session 3 (ADR-050 + smoke).
...
> **Actual sessions:** 3 / 4 (Phase 0 A-scope 2026-04-19; Session 1b 2026-04-20; Session 1c 2026-04-21)
...
> **ADR:** ADR-050 (Forgot-Password Role-Gate).
...
| 2.0.0   | YYYY-MM-DD | Phase 6 complete — ADR-050 written, shipped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Microsoft OAuth (ADR-046, v0.4.0 G3)                                                                                     | Orthogonal code path — no shared functions with `auth.service.ts` (grep-verified). ADR-050 must include a "Relationship to ADR-046" section stating that OAuth-only admins are a legitimate use case and the role-gate does not attempt to block them at the OAuth layer.                                                                                                                                                                                                                                                  | 6             |
...
- [x] **OAuth anchor (v0.4.0 G3)** — **DONE 2026-04-19:** grep for `forgotPassword|password_reset|resetPassword` in `backend/src/nest/auth/oauth/` → **0 matches**. Orthogonality claim for ADR-050 "Relationship to ADR-046" is anchored.
...
export function isAuthEndpoint(path: string): boolean {
  return (
    ...
    path.includes('/auth/forgot-password') || // NEW — ADR-050 two-gate coverage
    path.includes('/auth/reset-password') // NEW — ADR-050 two-gate coverage
  );
}
```
...
- [x] **OAuth-path grep recorded (v0.4.0 G3)** — **DONE 2026-04-19:** `grep -rE 'forgotPassword|password_reset|resetPassword' backend/src/nest/auth/oauth/` → **0 matches**. Orthogonality anchor for ADR-050 "Relationship to ADR-046" locked in.
...
| 2                   | 3 + 4 + 5                 | Unit tests (≥ 10 self-service + ≥ 7 admin-initiated = ~17) + API integration tests (≥ 8 self-service + ≥ 6 admin-initiated = ~14) + Frontend: §5.1+§5.2 merge-not-replace for `/forgot-password` + §5.3 `/manage-admins` button + §5.4 `/manage-employees` button.                                                                                                                                                                                                                                                                                                                                                                                                    | ⏳ PENDING |            |
| 3                   | 6                         | **ADR-050** (combined: Forgot-Password Role-Gate + Root-Initiated Reset, "Relationship to ADR-046" section, "Sequencing after ADR-049" section) + end-to-end smoke (dev + production) for all 6 flows (3 self-service paths + 3 admin-initiated paths). v0.4.3: no TRUNCATE deploy-step.                                                                                                                                                                                                                                                                                                                                                                              | ⏳ PENDING |            |
...
| D11 | v0.3.0 R7 "OAuth has no code collision"                                                                                                                                                    | Correct for code-level, missed the architectural-bypass angle                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | **v0.4.0 G3:** §0.2.5 #11 + ADR-050 "Relationship to ADR-046" acknowledge Microsoft OAuth as an orthogonal alternate-auth path for admin/employee.                                                                                                                                                                                                                                                           |
