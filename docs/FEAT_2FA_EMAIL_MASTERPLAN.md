...
- [x] **Grafana alert rule** (NOT Sentry — per ADR-002 Phase 5g, alerts are provisioned as code in `docker/grafana/alerts/*.json` via `apply.sh`): SMTP failure rate > 5 % over 5 min. New file `docker/grafana/alerts/08-smtp-failure-rate.json` shipped 2026-05-01 (v0.8.8); **deployed to Grafana Cloud 2026-05-01 (v0.8.12)** via `doppler run -- ./docker/grafana/alerts/apply.sh` (HTTP 201 Created — uid `assixx-smtp-failure-rate` in folder `assixx-prod-alerts`, group `assixx-warning`, `for: 5m`, threshold `> 0.05`). All 8 rules now live in Cloud. Two-query Loki ratio: numerator `count_over_time({…level="error"…} \|= "Fehler beim Senden der E-Mail" [5m])` (email-service.ts:612), denominator `+ {…level="info"…} \|= "E-Mail gesendet" [5m]` (email-service.ts:609); math node `$A / ($A + $B + 0.001)` — epsilon verhindert div-by-zero auf idle 5-min-Fenster. Pattern mirrors 04-backend-error-rate-warning + 07-tempo-cloud-quota-high.
- [ ] Sentry: 5xx errors from 2FA endpoints captured automatically by existing `all-exceptions.filter.ts` (no new wiring).
- [x] Grafana dashboard panel: 2FA verify success/fail rate per minute (`audit_trail` query) — `docker/grafana/dashboards/cloud/assixx-2fa.json` shipped 2026-05-01 (v0.8.11); **deployed to Grafana Cloud 2026-05-01 (v0.8.12)** via `doppler run -- grafanactl resources push --path docker/grafana/dashboards/cloud dashboards/assixx-2fa` (✔ 1 resource pushed, 0 errors; verified via `grafanactl resources get dashboards/assixx-2fa` → `Dashboard / assixx-2fa / stacks-1493105`). Format mirrors `assixx-logs-by-level.json` (Kubernetes-style for `grafanactl resources push --kind dashboard`, per HOW-TO-GRAFANA-CLI). Folder `assixx-dashboards`, datasource `grafanacloud-logs`, refresh 30s, 4 data panels in 3 rows: (1) **Verify Success/Fail per minute** — stacked bars green/red, queries grounded on §A8: success = `action="login" \| resource_type="auth" \| status="success" \| changes_method="2fa-email"`, fail = same with `status="failure" \| changes_reason=~"wrong-code\|expired-challenge"`; (2) **Challenges Issued per minute** — line chart `sum by (purpose)` over `resource_type="2fa-challenge" \| action="create"` (purpose ∈ login/signup/email-change-old/email-change-new); (3) **Lockouts Triggered per minute** — orange bars on `resource_type="2fa-lockout" \| action="update"`; (4) **Recent 2FA Audit Events** — log panel with the canonical Loki query from HOW-TO-ALERTING §5b. Drift caught + fixed: §5c §4 verify-rate recipe used non-existent `changes->>'outcome'` field — replaced with §A8-grounded `status` + `changes.method`/`changes.reason` (drift would have produced silent zero-row results in compliance audits). Memory `feedback_audit_tuples_a8_first.md` saved. Deployment via `doppler run -- grafanactl resources push --path docker/grafana/dashboards/cloud --kind dashboard` is **User-Only** (push to Grafana Cloud / shared external state).
- [x] Loki saved query: `{service="backend"} | json | resource_type="2fa-challenge" or resource_type="2fa-lockout"` — documented in `docs/how-to/HOW-TO-ALERTING.md` §5b "2FA Saved Loki Queries (ADR-054)" shipped 2026-05-01 (v0.8.9). Five variants: canonical (all gating events), lockout-only (DD-5/DD-6 brute-force detection), production-filter (cuts dev-noise via `app="assixx", env="production"`), per-tenant breakdown (`sum by (tenant_id)` aggregation), Tempo click-through pointer (ADR-048 derived-field). Cross-links: ADR-054 §A8 audit tuples + alert 08-smtp-failure-rate + masterplan Phase 6. Rationale for "doc instead of Grafana Library Panel": Explore-stars are UI-only / ephemeral; Library Panels overkill for ad-hoc snippets; code-tracked = single source of truth + survives Cloud-account migration.
- [x] Audit-trail filter recipe: `WHERE resource_type IN ('2fa-challenge','2fa-lockout') OR (action='login' AND changes->>'method'='2fa-email')` — documented in `docs/how-to/HOW-TO-ALERTING.md` §5c "2FA Audit-Trail Filter Recipes (ADR-054 / ADR-009)" shipped 2026-05-01 (v0.8.10). Five SQL recipes: canonical filter (returns full event row), 24h-lockout report (`HAVING COUNT(*) >= 2` candidates for HOW-TO-2FA-RECOVERY intervention), per-tenant 7-day volume (capacity + SMTP-cost signal via `challenges >> verified_logins` divergence), hourly verify-fail rate (complements SMTP-fail-rate alert — distinguishes transport vs human-error spikes), GDPR Art.15 single-user export. Cross-links: ADR-009 audit-trail interceptor + partitioning, ADR-054 §A8 audit tuples, ADR-029 pg_partman, HOW-TO-2FA-RECOVERY. Connection-mode hint: `assixx_user` BYPASSRLS for cross-tenant; `app_user` + `SET app.tenant_id` for scoped reads.
