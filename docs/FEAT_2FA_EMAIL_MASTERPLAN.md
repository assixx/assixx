...
> **Plan type:** FEATURE
> **Created:** 2026-04-26
> **Version:** 0.5.0 (HARDENED — DD-10 Flag entfernt per User-Vorgabe „kein Einstellung", Throttler P2-Fix, Production-SMTP-Smoke + Sender-Warmup hinzugefügt)
...
| DD-31 | Post-Cutover-Monitoring-Fenster | **T+1 Quick-Review + T+7 Stable-State-Review.** T+1 = SMTP-Fail-Rate, Lockout-Count, Support-Tickets. T+7 = vollständiger Review aller Metriken. Wenn alles grün → erhöhtes Monitoring beenden, Standard-Alerting ausreichend. | Plan-Default (T+1) war zu knapp — Lockouts werden bei wenig-aktiven Usern erst Tage später sichtbar. T+7 fängt diese Spät-Effekte ab. | **APPROVED** (v0.4.0) |
...
### Step 0.3: Pre-deploy Tenant-Mail (T-7 Vorabmail) [PENDING]

> **v0.4.0 Konkretisierung:** alle Empfänger-, Sprache-, Tonalitäts- und Format-Entscheidungen in DD-26 + DD-27 fixiert.
...
> **Hard rule v0.5.0:** Steps 0.5.3 und 0.5.5 müssen DONE sein bevor Phase 2 startet (block code work). Steps 0.5.1, 0.5.2, 0.5.4, **0.5.6, 0.5.7** müssen DONE sein vor T-Day (block production cut-over). DD-10-Removal verschärft 0.5.6 + 0.5.7 zu Hard-Blockern, nicht Soft-Recommendations.
...
- [ ] **load tests** (`load/lib/auth.ts`, `load/tests/baseline.ts`) angepasst auf Discriminated-Union — fail-loud bei `stage === 'challenge_required'` (R13-Mitigation v0.5.0 — load smoke MUSS grün vor T-0)
...
- [ ] Pre-deploy email drafted + sent to all tenants 7 days before T-Day Deploy (v0.5.0: kein Flag-Flip — Deploy = Cutover)
...
### Cut-over runbook (v0.5.0 — DD-22 revised, DD-23, DD-28, DD-31, DD-10 REMOVED)

**T-Day-Setup:** T-Day wird gesetzt nach Phase 5 DoD (DD-22) = Phase 5 Completion + 14 Tage Puffer. Alle T-X Schritte sind **relativ** zu diesem T-Day.

| Tag          | Aktion                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **T-14**     | Single-Root-Detection #1 + 1. Outreach-Mail an alle Single-Root-Tenants (Step 0.5.1, DD-28)                                                                                                                                                                                                                                                                                                                                                           |
| **T-7**      | (a) Single-Root-Detection #2 + 2. Outreach-Mail. (b) Vorabmail an alle Tenant-Roots (Step 0.3, DD-26 + DD-27).                                                                                                                                                                                                                                                                                                                                        |
| **T-1**      | (a) Single-Root-Detection #3. (b) **Hard-Block-Check (DD-23):** wenn Liste nicht-leer → T-Day um 7 Tage verschieben + E-Mail-Eskalation an betroffene Roots. KEIN Telefon. (c) Wenn T-Day hält: **v0.5.0: Production-SMTP-Smoke (Step 0.5.6) als finalen Pre-Deploy-Test** ausführen — echter Code-Send an Test-Mailbox bei Outlook/Gmail/GMX. Subdomain-DNS+Cert-Check (Step 0.5.4) re-verifizieren. Sender-Warmup (Step 0.5.7) Day-7 abgeschlossen. |
| **T-0**      | **v0.5.0: Code-Deploy = 2FA sofort live für alle** (kein Flag-Flip mehr — DD-10 entfernt). Engmaschiges Monitoring: SMTP-Fail-Rate, Lockout-Count, Support-Tickets, Spam-Folder-Hit-Rate. Bei kritischen Issues: SSH-basierter Code-Revert + Redeploy als einziger Rollback (~10–30 Min, §0.1a Out-of-Band-Pfade).                                                                                                                                    |
| **T+1 Tag**  | Quick-Review Metriken (DD-31): SMTP-Fail-Rate < 5 %, keine 5xx-Spikes, Lockout-Count < 1 % MAU                                                                                                                                                                                                                                                                                                                                                        |
| **T+7 Tage** | **Stable-State-Review (DD-31):** vollständige Metriken-Auswertung. Wenn alle grün → erhöhtes Monitoring beenden, Standard-Alerting reicht.                                                                                                                                                                                                                                                                                                            |
