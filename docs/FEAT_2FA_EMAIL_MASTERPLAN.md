...
> **Plan type:** FEATURE
> **Created:** 2026-04-26
> **Version:** 0.4.0 (PERFECTED — alle operativen Entscheidungen aufgelöst, Plan implementierungsreif, keine offenen Fragen mehr)
...
| DD-22 | Cutover-Strategie (T-Day) | **T-Day = Phase 5 Completion + 14 Tage Puffer.** Konkretes Datum erst gesetzt, wenn Phase 5 DoD grün ist. 7 Tage davon = Vorabmail-Frist (Step 0.3), die anderen 7 Tage = Soft-Rollout mit `FEATURE_2FA_EMAIL_ENFORCED=false` zur Code-Validation ohne Verhaltens-Change. Vorabmail wird in Phase 6 gedraftet (NICHT vorher), damit das Datum stimmt. | Niedrigstes Risiko. Keine Verschiebung wegen Implementierungs-Verzögerungen. Vorabmail nur einmal verschickt (kein Re-Issue wegen verschobenem Datum). | **APPROVED** (v0.4.0) |
| DD-23 | Per-Tenant 2FA-Flag in V1 | **NEIN.** DD-10 bleibt single-global (`FEATURE_2FA_EMAIL_ENFORCED` env-flag). Falls am T-1 Single-Root-Tenants nicht reagiert haben → **Cutover um 7 Tage verschieben + E-Mail-Eskalation an alle betroffenen Roots** (KEIN Telefon-Support). Wenn auch nach 7-Tage-Verschiebung Tenant nicht reagiert → eskalieren als Produkt-Entscheidung. | KISS für V1. Per-Tenant-Flag ist V2 wenn ein konkreter Bedarfsträger es braucht. E-Mail-only-Eskalation passt zu B2B-SaaS-Kommunikationsmodell. | **APPROVED** (v0.4.0) |
...
### Cut-over runbook (v0.4.0 — DD-22, DD-23, DD-28, DD-31)
...
| **T-1**      | (a) Single-Root-Detection #3. (b) **Hard-Block-Check (DD-23):** wenn Liste nicht-leer → Cutover um 7 Tage verschieben + E-Mail-Eskalation an betroffene Roots. KEIN Telefon. (c) Wenn T-Day hält: Deploy Code mit `FEATURE_2FA_EMAIL_ENFORCED=false` → Smoke-Test (kein Verhaltens-Change). |
| **T-0**      | Flag flippen (`FEATURE_2FA_EMAIL_ENFORCED=true`) → engmaschiges Monitoring (SMTP-Fail-Rate, Lockout-Count, Support-Tickets)                                                                                                                                                                 |
