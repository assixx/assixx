...
> **Plan type:** FEATURE
> **Created:** 2026-04-16
> **Version:** 0.3.3 (Fifth-pass hardening — §2.8b OAuth seeder now wraps the `tenant_domains` INSERT in a try/catch for PG `23505` → `ConflictException({ code: 'DOMAIN_ALREADY_CLAIMED' })`; required consequence of v0.3.2's UNIQUE index promotion; Phase 3 + Phase 4 test mandates added so the conflict path is actually exercised, not just documented)
> **Status:** DRAFT — Phase 0 (current-state discovery) not started
...
- [ ] `checkSmtp: false` assertion present
...
- [ ] Rate-limit present on domains endpoints (quick sanity test: 100 requests in 1 min → 429 at some point)
