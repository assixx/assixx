...
> **Plan type:** FEATURE
> **Created:** 2026-04-21
> **Version:** 0.2.0 (Draft — post-verification revision)
> **Status:** DRAFT — Phase 0 (planning, awaiting sign-off)
...
| R13 | `shift_handover_slot` was proposed as a hard ENUM but `shift_times.shift_key` is tenant-configurable VARCHAR(20)            | High   | High        | DDL uses `shift_key VARCHAR(20) NOT NULL` + `CHECK (shift_key IN ('early','late','night'))` for V1 only. App-layer also validates that `shift_key` exists in `shift_times` for the tenant. V1 limitation documented in §Known Limitations: tenants with custom shift_key values (e.g. `'F','S','N'`, `'morgen','tag','nacht'`) are unsupported until V2. **No new DB enum type created** — the existing `shifts_type` enum is unrelated (14 labor-law values, not slot keys) and MUST NOT be reused for handover identity. | `SELECT shift_key FROM shift_times GROUP BY shift_key;` across test + any prod-like tenant returns only `('early','late','night')` — otherwise V1 launch is blocked for that tenant. |
...
- [ ] Decision recorded here: **\_** (TBD after spike)
- [ ] Verify the load-bearing `@fastify/multipart` registration in `backend/src/nest/main.ts` (currently line 175, previously `:99` in ADR-042 text) is still present with its pointer comment after our work. Do not regress.

> Time-box: 1 hour. If spike runs over → escalate, do not solve in Phase 1.
> **Pre-spike default:** ADAPT. Inventory's photo table is item-coupled and the field-naming differs (`item_id` vs `entry_id`). Refactor is rarely worth it for two consumers.
...
- [ ] Decision recorded here: **\_\_\_\_** (TBD after spike)
- [ ] Output: one pure function signature that Phase 2.3 implements:
  ```typescript
...
> **Pre-spike default:** assume test-tenant parity. If the spike confirms it, Phase 1.2 uses `CHECK (shift_key IN ('early','late','night'))`. Else Phase 1.2 is blocked pending product re-scope.
...
### Step 1.1: Create `shift_handover_templates` [PENDING]
...
### Step 1.2: Create `shift_handover_entries` [PENDING]
...
### Step 1.3: Create `shift_handover_attachments` [PENDING]
...
- [ ] 2 or 3 migration files (depends on §0.7) with both `up()` AND `down()`
- [ ] Backup taken: `database/backups/full_backup_pre_shift_handover_{ts}.dump`
- [ ] Dry run passes: `doppler run -- ./scripts/run-migrations.sh up --dry-run`
- [ ] All migrations applied; entries in `pgmigrations`
- [ ] All tables verified: `\d shift_handover_*` shows expected columns + RLS
- [ ] Policies verified: `SELECT tablename, policyname FROM pg_policies WHERE tablename LIKE 'shift_handover_%';` returns one per table
- [ ] GRANTs verified for both `app_user` and `sys_user`
- [ ] Backend compiles after migration: `docker exec assixx-backend pnpm run type-check` → 0 errors
...
### Step 2.1: Module skeleton + types + DTOs + shared field schemas [PENDING]
...
- [ ] Create `shared/src/shift-handover/` directory + 3 files.
- [ ] Add the `"./shift-handover"` block to `shared/package.json` exports.
- [ ] Run `pnpm --filter @assixx/shared build` — must produce `shared/dist/shift-handover/{index,field-types,field-validators}.{js,d.ts}`.
- [ ] Run `pnpm run sync:svelte` (ADR-041 Related Fix — ensures `.svelte-kit/tsconfig.json` regenerates before frontend tooling resolves the new import).
- [ ] Verify resolution: `docker exec assixx-backend pnpm run type-check` + `cd frontend && pnpm run check` both return 0 errors with an `import { buildEntryValuesSchema } from '@assixx/shared/shift-handover';` somewhere in touched code.

**Register in `app.module.ts`:**

- [ ] `ShiftHandoverModule` added to imports (alphabetical)

### Step 2.2: `ShiftHandoverTemplatesService` [PENDING]
...
### Step 2.3: `ActiveShiftResolverService` [PENDING]
...
### Step 2.4: `ShiftHandoverAttachmentsService` [PENDING]

**Implementation depends on §0.7 decision.** Either thin wrapper around inventory service OR full copy.

**Methods:**

- `uploadForEntry(entryId, file)` — enforces 5-image cap, mime whitelist (image/jpeg, image/png, image/webp, image/heic), 10 MB size cap
...
### Step 2.5: `ShiftHandoverEntriesService` [PENDING] — the heart
...
| ------- | ----- | ----------------------------------------------------------------------------- | ------- | ---- |
| 1       | 0     | §0.7 Inventory spike + §0.8 slot-identity spike + risk register sign-off      | PENDING |      |
| 2       | 1     | Migrations: templates + entries (+ attachments if adapt); uuidv7() verified   | PENDING |      |
| 3       | 2     | Module skeleton + DTOs + shared field schemas + `shared/package.json` exports | PENDING |      |
| 4       | 2     | TemplatesService + ActiveShiftResolver (uses §0.8 spike output)               | PENDING |      |
| 5       | 2     | EntriesService + AttachmentsService                                           | PENDING |      |
