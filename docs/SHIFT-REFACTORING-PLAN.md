...
export async function loadShiftsForUser(userId: number, tenantId: number): Promise<Shift[]> {
  ...
  const accessibleDeptIds = await hierarchyPermissionService.getAccessibleIds(userId, tenantId, 'department');
...
_Dieser Plan wurde erstellt basierend auf einer vollständigen Analyse aller Dateien. Bei Fragen oder Unklarheiten: FRAGEN, nicht raten._
