...
| **`pnpm run db:seed` → `could not translate host name "assixx-postgres"`** | Doppler dev-config injiziert `DB_HOST=assixx-postgres` (Docker-Container-Name). Vom Host aus nicht auflösbar — nur innerhalb des Compose-Netzwerks. | `scripts/run-seeds.sh` redirected `assixx-postgres` und `postgres` automatisch nach `localhost` (case-Statement). Falls weiterhin Fehler: Direkter Fallback via `docker exec -i assixx-postgres psql -U assixx_user -d assixx -v ON_ERROR_STOP=1 < database/seeds/<file>.sql` pro Seed-Datei. |
