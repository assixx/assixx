...
```sql
-- Alle Features für brunotest-Tenant aktivieren
...
├── environments/                  # Bruno env files (Legacy, nur Referenz)
└── auth/, users/, ...             # Bruno .bru files (Legacy, bis Cleanup)
```
...
let auth: AuthState;
beforeAll(async () => {
  auth = await loginBrunotest();  // → cached nach erstem Aufruf
});
```
...
| `loginBrunotest()`     | `() => Promise<AuthState>`                   | Cached Login — EIN HTTP-Request für gesamte Suite            |
...
Vollständige Dokumentation der Migration von Bruno CLI zu Vitest:
