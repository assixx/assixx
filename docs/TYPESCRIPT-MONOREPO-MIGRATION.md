...
import { ApiClient } from '../../utils/api-client';  // ❌ ERROR!
...
```
*(Added to both `backend` and `deletion-worker` services)*
...
```typescript
// backend/src/app.ts
import { db } from './database.js';  // ← Must use .js!
...
import { ApiClient } from '../../utils/api-client';  // ← No .js needed!
