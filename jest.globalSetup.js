...
export default async function globalSetup() {
  ...
  try {
    ...
    if (before[0].count > 0) {
      console.log(`⚠️  Found ${before[0].count} leftover test tenants. Cleaning up...`);
      ...
      await db.execute(`DELETE FROM users WHERE username LIKE '__AUTOTEST__%' OR email LIKE '__AUTOTEST__%'`);
      await db.execute(`DELETE FROM tenants WHERE subdomain LIKE '__AUTOTEST__%' OR company_name LIKE '__AUTOTEST__%'`);
