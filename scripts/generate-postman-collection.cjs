...
const outputPath = path.join(__dirname, '../docs/assixx-api.postman_collection.json');
...
console.info(`   - API Groups: ${Object.values(folders).filter((f) => f.item.length > 0).length}`);
console.info(`\n🚀 Import in Postman:`);
