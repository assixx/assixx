    ...
    errors.forEach((error) => {
      ...
      if (error.ruleId === '@typescript-eslint/no-floating-promises') {
        ...
        const patterns = [
          ...
          {
            pattern: /^(\s*)([\w.]+)\((.*)\);?\s*$/,
            replacement: (match, indent, func, args) => `${indent}void ${func}(${args});`,
          },
          // Method calls that return promises
          {
            pattern: /^(\s*)([\w.]+\.[\w.]+)\((.*)\);?\s*$/,
            replacement: (match, indent, method, args) => `${indent}void ${method}(${args});`,
          },
          // Async function calls in if statements
          {
            pattern: /^(\s*)if\s*\((.*)\)\s*{\s*$/,
            check: (match, indent, condition) => condition.includes('(') && condition.includes(')'),
            skip: true, // These need manual review
          },
          // Promise chains
          {
            pattern: /^(\s*)(.*\.)then\(/,
            replacement: (match, indent, prefix) => `${indent}void ${prefix}then(`,
          },
          // setTimeout/setInterval with async functions
          {
            pattern: /^(\s*)(setTimeout|setInterval)\(\s*async\s*/,
            replacement: (match, indent, timer) => `${indent}void ${timer}(async `,
          },
        ];
        ...
        if (!fixed) {
          ...
          if (
            ...
            lines[lineIndex] = line.replace(/(\s*)loadShiftData\(\)/, '$1void loadShiftData()');
            fixedCount++;
          }
          ...
          // initialize/init type calls
            ...
            lines[lineIndex] = line.replace(/(\s*)((initialize|init)[\w]*\()/, '$1void $2');
            fixedCount++;
          }
        }
        ...
        // Event handlers with async functions
          ...
          lines[lineIndex] = line.replace(/addEventListener\([^,]+,\s*async\s*/, (match) =>
            match.replace('async ', '(e) => { void (async () => '),
          );
          fixedCount++;
        }
        ...
        if (line.includes('await') && line.includes('redisClient.quit()')) {
          lines[lineIndex] = line.replace('await redisClient.quit()', 'redisClient.quit()');
          fixedCount++;
        }
      }
    });
