...
export function hasUnsavedChanges(
  ...
  if (originalWeeklyShifts === null) {
    // No original data - check if current has any shifts
  ...
  for (const [date, dayShifts] of weeklyShifts) {
    ...
    if (dayShifts.size !== originalDayShifts.size) return true;

    for (const [shiftType, userIds] of dayShifts) {
      const originalUserIds = originalDayShifts.get(shiftType);
      if (originalUserIds === undefined) return true;

      if (userIds.length !== originalUserIds.length) return true;

      const sortedCurrent = [...userIds].sort();
      const sortedOriginal = [...originalUserIds].sort();
 ...
 */
export function handleDropdownSelect<T extends { id: number; name: string }>(
  item: T,
...
export function addEmployeeToShiftMap(
  ...
  if (!newShifts.has(dateKey)) {
    newShifts.set(dateKey, new Map());
  }
