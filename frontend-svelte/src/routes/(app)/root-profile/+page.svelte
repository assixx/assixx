<script>
  ...
  // Local modules
  import { STORAGE_KEYS, MESSAGES, PASSWORD_TOOLTIP, PASSWORD_RULES } from './_lib/constants';
  import {
    loadProfile as apiLoadProfile,
    loadProfilePicture as apiLoadProfilePicture,
    loadPendingApprovals as apiLoadPendingApprovals,
  ...
  /** @typedef {import('./_lib/types').UserProfile} UserProfile */
  /** @typedef {import('./_lib/types').ApprovalItem} ApprovalItem */
  /** @typedef {import('./_lib/types').PasswordStrengthResult} PasswordStrengthResult */
  /** @typedef {import('./_lib/types').PasswordField} PasswordField */
  ...
  let user = $state(/** @type {UserProfile | null} */ (null));
  ...
  // Password Strength
  /** @type {PasswordStrengthResult | null} */
  ...
  async function checkAuth() {
    try {
      ...
      if (role !== 'root') {
        goto(`${base}/${role}-profile`);
        return;
      }

      await Promise.all([loadProfile(), loadApprovals()]);
    } catch (err) {
      console.error('[RootProfile] Auth check failed:', err);
      goto(`${base}/login`);
    }
  }
  ...
  async function loadProfile() {
    ...
    const result = await apiLoadProfile();
    user = result.user;
    error = result.error;
  ...
  async function loadApprovals() {
    pendingApprovals = await apiLoadPendingApprovals();
  }
  ...
  async function saveProfile(event) {
    ...
    try {
      ...
      if (user) {
        user = { ...user, email: formEmail, firstName: formFirstName, lastName: formLastName };
      }
